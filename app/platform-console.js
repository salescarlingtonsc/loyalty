(function platformConsoleFactory(globalObject) {
  ...
  const PLATFORM_COPY_STRICT=Object.freeze({
    'zh-CN':Object.freeze({
      ...
      'Admin action':'管理员操作',
      'Admin decision':'管理员决定',
      'Admin decision saved.':'管理员决定已保存。',
      'Approved but not activated':'已批准但未启用',
      'Approved by platform admin. Workspace access still requires business details and verified payment or manual billing.':'平台管理员已批准。使用工作区仍需填写商户资料，并完成付款验证或人工计费。',
      'Archive record':'归档记录',
      'Archive this record':'归档此记录',
      'Archived and merged-away firm records will appear here.':'已归档和已合并的企业记录会显示在这里。',
      'Archived and merged-away firms are hidden from the board and every count. Nothing here is deleted.':'已归档和已合并的企业不会出现在看板和任何统计中。这里的内容都没有被删除。',
      'Archived firm records':'已归档的企业记录',
      'Archived records unavailable':'无法加载已归档记录',
      'Business owner signups':'企业负责人注册',
      'By':'操作人',
      'Choose Approve, Reject or Pending.':'请选择批准、拒绝或待定。',
      'Choose the stage for {name}. The next step records the evidence for that stage.':'请为 {name} 选择阶段。下一步将记录该阶段所需的证据。',
      'Contacts, activities, tasks, documents and tags move to the firm you choose. {name} is then archived and cannot be restored on its own. Stage history and evidence stay with {name}, so the audit trail is not rewritten.':'联系人、活动、任务、文件和标签会转移到您选择的企业。{name} 随后被归档，且无法单独恢复。阶段历史和证据仍留在 {name}，因此审计记录不会被改写。',
      'Creates the workspace, the owner staff record and the default branch for the confirmed owner account, then marks the application consumed. The owner can sign in immediately afterwards. No payment is recorded and no invoice is marked paid.':'为已确认的负责人账号创建工作区、负责人员工记录和默认分店，然后将该申请标记为已使用。负责人随后即可登录。不会记录任何付款，也不会将发票标记为已付。',
      'Demo':'演示',
      'Demo firm':'演示企业',
      'Demo firms stay visible in the directory but are excluded from the daily billing-lifecycle scan and platform revenue/aggregate reports.':'演示企业仍会显示在目录中，但不纳入每日计费周期扫描，也不计入平台收入和汇总报告。',
      'Drag a card into another column to move it, or focus a card and press Ctrl with the left or right arrow. Every move still asks for its evidence.':'将卡片拖到另一列即可移动，或选中卡片后按 Ctrl 加左右方向键。每次移动仍会要求提供证据。',
      'Excluded from billing exceptions and revenue rollups.':'不纳入计费异常和收入汇总。',
      'Generate AI report':'生成 AI 报告',
      'Generate now':'立即生成',
      'If the connection is interrupted, press Activate workspace again. The same activation returns the existing workspace instead of duplicating it.':'若连接中断，请再次点击启用工作区。重复启用会返回已有的工作区，不会重复创建。',
      'Keep this firm':'保留此企业',
      'Last contacted':'最后联系',
      'Last: {when}':'最后：{when}',
      'Live':'正式',
      'Loading archived firm records\\u2026':'正在加载已归档的企业记录…',
      'Merge into another firm':'合并到另一家企业',
      'Merge records':'合并记录',
      'Merged into {name}':'已合并到 {name}',
      'Merged. {contacts} contacts and {activities} activities moved to the surviving firm.':'已合并。{contacts} 位联系人和 {activities} 条活动已转移到保留的企业。',
      'Move to {lane}':'移动到{lane}',
      'Next: {when}':'下一步：{when}',
      'No confirmed owner account matches this application email. Ask the owner to sign in or confirm their email, then approve again.':'没有与此申请邮箱匹配的已确认负责人账号。请让负责人登录或确认邮箱后再次批准。',
      'Only a period that has already closed can be reported on. Demo firms are excluded.':'只能对已结束的周期生成报告。演示企业不包含在内。',
      'Optional. Recorded in the audit log.':'可不填。会记录在审计日志中。',
      'Outcome notes':'结果备注',
      'Pending admin decision. Workspace access remains closed.':'等待管理员决定。工作区暂不可使用。',
      'Period starts on':'周期开始于',
      'Personal data erased.':'个人资料已删除。',
      'Queue report':'加入报告队列',
      'Quotation amount (SGD)':'报价金额（新元）',
      'Read report':'查看报告',
      'Record':'记录',
      'Recorded in the audit log \\u2014 for example, a PDPA erasure request.':'会记录在审计日志中，例如 PDPA 删除请求。',
      'Recorded in the audit log.':'会记录在审计日志中。',
      'Rejected by platform admin. Workspace access remains closed.':'平台管理员已拒绝。工作区不可使用。',
      'Report period':'报告周期',
      'Restore record':'恢复记录',
      'Sent at':'发送时间',
      'Show demo firms':'显示演示企业',
      'The AI report is queued. It appears here once it is written.':'AI 报告已加入队列。生成后会显示在这里。',
      'The contact row, the firm and the audit trail remain, so the record of the erasure itself survives.':'联系人记录、企业和审计记录都会保留，因此这次删除本身也有据可查。',
      'There is no other firm record to merge into.':'没有其他企业记录可供合并。',
      ...
      'Why is this being erased?':'为什么要删除？',
      'Why is this being restored?':'为什么要恢复？',
      'Written every month, quarter and year from the same evidence as the consultant brief. New customers, sales growth and account opens are always covered, and no number is invented.':'每月、每季度和每年生成，依据与顾问简报相同的证据。始终涵盖新顾客、销售增长和开户情况，且不会编造任何数字。',
      'Yearly':'每年',
      'another firm':'另一家企业',
      ...
      '{name} is removed from the board, the list and every count. Nothing is deleted — its history and audit trail are kept, and a super admin can restore it.':'{name} 会从看板、列表和所有统计中移除。任何内容都不会被删除：其历史和审计记录都会保留，超级管理员可以恢复。',
      '{name} restored.':'{name} 已恢复。',
      '{name} returns to the board and to every count, with its history intact.':'{name} 会回到看板和所有统计中，历史记录完好保留。',
    ...
    ms:Object.freeze({
      ...
      'Admin action':'Tindakan pentadbir',
      'Admin decision':'Keputusan pentadbir',
      'Admin decision saved.':'Keputusan pentadbir disimpan.',
      'Approved but not activated':'Diluluskan tetapi belum diaktifkan',
      'Approved by platform admin. Workspace access still requires business details and verified payment or manual billing.':'Diluluskan oleh pentadbir platform. Akses ruang kerja masih memerlukan butiran perniagaan dan pembayaran yang disahkan atau pengebilan manual.',
      'Archive record':'Arkibkan rekod',
      'Archive this record':'Arkibkan rekod ini',
      'Archived and merged-away firm records will appear here.':'Rekod firma yang diarkib dan digabungkan akan muncul di sini.',
      'Archived and merged-away firms are hidden from the board and every count. Nothing here is deleted.':'Firma yang diarkib dan digabungkan disembunyikan daripada papan dan semua kiraan. Tiada apa-apa di sini dipadam.',
      'Archived firm records':'Rekod firma yang diarkib',
      'Archived records unavailable':'Rekod diarkib tidak tersedia',
      'Business owner signups':'Pendaftaran pemilik perniagaan',
      'By':'Oleh',
      'Choose Approve, Reject or Pending.':'Pilih Lulus, Tolak atau Menunggu.',
      'Choose the stage for {name}. The next step records the evidence for that stage.':'Pilih peringkat untuk {name}. Langkah seterusnya merekodkan bukti bagi peringkat itu.',
      'Contacts, activities, tasks, documents and tags move to the firm you choose. {name} is then archived and cannot be restored on its own. Stage history and evidence stay with {name}, so the audit trail is not rewritten.':'Kenalan, aktiviti, tugasan, dokumen dan tag berpindah ke firma yang anda pilih. {name} kemudian diarkibkan dan tidak boleh dipulihkan sendiri. Sejarah peringkat dan bukti kekal dengan {name}, jadi jejak audit tidak ditulis semula.',
      'Creates the workspace, the owner staff record and the default branch for the confirmed owner account, then marks the application consumed. The owner can sign in immediately afterwards. No payment is recorded and no invoice is marked paid.':'Mencipta ruang kerja, rekod kakitangan pemilik dan cawangan lalai untuk akaun pemilik yang disahkan, kemudian menandakan permohonan sebagai digunakan. Pemilik boleh log masuk sejurus selepas itu. Tiada pembayaran direkodkan dan tiada invois ditanda sebagai dibayar.',
      'Demo':'Demonstrasi',
      'Demo firm':'Firma demo',
      'Demo firms stay visible in the directory but are excluded from the daily billing-lifecycle scan and platform revenue/aggregate reports.':'Firma demo kekal kelihatan dalam direktori tetapi dikecualikan daripada imbasan kitaran pengebilan harian serta laporan hasil dan agregat platform.',
      'Drag a card into another column to move it, or focus a card and press Ctrl with the left or right arrow. Every move still asks for its evidence.':'Seret kad ke lajur lain untuk memindahkannya, atau fokus pada kad dan tekan Ctrl bersama anak panah kiri atau kanan. Setiap pemindahan tetap meminta buktinya.',
      'Excluded from billing exceptions and revenue rollups.':'Dikecualikan daripada pengecualian pengebilan dan rumusan hasil.',
      'Generate AI report':'Jana laporan AI',
      'Generate now':'Jana sekarang',
      'If the connection is interrupted, press Activate workspace again. The same activation returns the existing workspace instead of duplicating it.':'Jika sambungan terputus, tekan Aktifkan ruang kerja sekali lagi. Pengaktifan yang sama mengembalikan ruang kerja sedia ada dan tidak menduanya.',
      'Keep this firm':'Kekalkan firma ini',
      'Last contacted':'Kali terakhir dihubungi',
      'Last: {when}':'Terakhir: {when}',
      'Live':'Langsung',
      'Loading archived firm records\\u2026':'Memuatkan rekod firma yang diarkib…',
      'Merge into another firm':'Gabung ke firma lain',
      'Merge records':'Gabungkan rekod',
      'Merged into {name}':'Digabungkan ke {name}',
      'Merged. {contacts} contacts and {activities} activities moved to the surviving firm.':'Digabungkan. {contacts} kenalan dan {activities} aktiviti dipindahkan ke firma yang dikekalkan.',
      'Move to {lane}':'Pindah ke {lane}',
      'Next: {when}':'Seterusnya: {when}',
      'No confirmed owner account matches this application email. Ask the owner to sign in or confirm their email, then approve again.':'Tiada akaun pemilik disahkan yang sepadan dengan e-mel permohonan ini. Minta pemilik log masuk atau sahkan e-mel mereka, kemudian luluskan semula.',
      'Only a period that has already closed can be reported on. Demo firms are excluded.':'Hanya tempoh yang telah tamat boleh dilaporkan. Firma demo dikecualikan.',
      'Optional. Recorded in the audit log.':'Pilihan. Direkodkan dalam log audit.',
      'Outcome notes':'Nota keputusan',
      'Pending admin decision. Workspace access remains closed.':'Menunggu keputusan pentadbir. Akses ruang kerja kekal tertutup.',
      'Period starts on':'Tempoh bermula pada',
      'Personal data erased.':'Data peribadi dipadam.',
      'Queue report':'Beratur laporan',
      'Quotation amount (SGD)':'Jumlah sebut harga (SGD)',
      'Read report':'Baca laporan',
      'Record':'Rekod',
      ...
      'Recorded in the audit log.':'Direkodkan dalam log audit.',
      'Rejected by platform admin. Workspace access remains closed.':'Ditolak oleh pentadbir platform. Akses ruang kerja kekal tertutup.',
      'Report period':'Tempoh laporan',
      'Restore record':'Pulihkan rekod',
      'Sent at':'Dihantar pada',
      'Show demo firms':'Tunjukkan firma demo',
      'The AI report is queued. It appears here once it is written.':'Laporan AI dalam barisan. Ia akan muncul di sini setelah ditulis.',
      'The contact row, the firm and the audit trail remain, so the record of the erasure itself survives.':'Baris kenalan, firma dan jejak audit kekal, jadi rekod pemadaman itu sendiri terpelihara.',
      'There is no other firm record to merge into.':'Tiada rekod firma lain untuk digabungkan.',
      ...
      'Why is this being erased?':'Mengapa ini dipadam?',
      'Why is this being restored?':'Mengapa ini dipulihkan?',
      'Written every month, quarter and year from the same evidence as the consultant brief. New customers, sales growth and account opens are always covered, and no number is invented.':'Ditulis setiap bulan, suku tahun dan tahun daripada bukti yang sama seperti taklimat perunding. Pelanggan baharu, pertumbuhan jualan dan pembukaan akaun sentiasa diliputi, dan tiada nombor direka.',
      'Yearly':'Tahunan',
      ...
      '{name} archived.':'{name} diarkibkan.',
      '{name} is removed from the board, the list and every count. Nothing is deleted — its history and audit trail are kept, and a super admin can restore it.':'{name} dikeluarkan daripada papan, senarai dan semua kiraan. Tiada apa-apa dipadam — sejarah dan jejak auditnya dikekalkan, dan super admin boleh memulihkannya.',
      '{name} restored.':'{name} dipulihkan.',
      '{name} returns to the board and to every count, with its history intact.':'{name} kembali ke papan dan ke semua kiraan, dengan sejarahnya utuh.',
      '{status} · {date}':'{status}, {date}',
    ...
    return `<section class="platform-route-note platform-status-note" style="margin-top:14px">
      ${CUI.icon('staff',{size:19})}<div style="width:100%">
        ...
        <div class="platform-card-list" style="margin-top:12px">${rows.map(item=>{
          ...
          const decision=triage==='contacted'
            ?pt('Approved')
            :triage==='archived'
              ?pt('Rejected')
              :pt('Pending');
