-- FRENLY v23g — LOYALTY MODEL CONSOLIDATION (review findings #1–#4, owner-approved).
-- Applied migration name: frenly_v23g_loyalty_model_consolidation
--
-- Review verdict on v23: multi-tenant lego pieces existed but did not interlock. This migration
-- makes the engine coherent while keeping EVERY strategy decision per-firm (nothing set in stone):
--
--   #1 P0  redeem_reward now drains points_batches FEFO (mirrors redeem_points exactly, including
--          the "batches must prove the balance" check) — expiry sweep, redeem_points and the
--          catalog path now share one truth. Verified drift before fix: ledger 55 vs batches 75.
--   #2 P1  Tier points_multiplier is WIRED: app.on_sale_recorded applies the client's current
--          tier multiplier at earn time (floor(base * multiplier)). "Gold = 2x" now actually happens.
--          Minimal diff of the live body; retention/referral logic untouched.
--   #3/#4  ONE ACTIVE REDEEM MODEL PER FIRM, firm-selected via loyalty_programs.loyalty_model:
--            'classic'      -> fixed redeem_points -> fixed credit (today's UI; the default)
--            'points_tiers' -> owner's reward catalog via redeem_reward (+ tiers)
--          Each RPC politely rejects when the firm runs the other model, so balances can't be
--          double-managed. Existing firms are backfilled to 'classic' (that is what the live UI
--          does today). Dead column earn_rate_bps dropped (0 references in functions or frontend;
--          earn_points_per_dollar is the live earn knob).
--
-- Everything remains owner-tunable: model choice, earn rate, redeem cost/value, expiry mode/days,
-- tier basis (visits/spend/points_earned), tier names/thresholds/multipliers, reward catalog.
