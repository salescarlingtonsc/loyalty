-- PEEKAA v143 - EFFECTIVE REWARD AND TIER BOUNDARIES
--
...
-- exclusive window and adds bounded owner/customer RPCs for authoring and
-- projection. No production action is authorized by this file.
...
begin
  ...
  perform pg_advisory_xact_lock(hashtextextended(
    'v143-tier-defaults:'||p_business::text||':'||v_actor::text||':'||p_idempotency_key::text,0
  ));
  perform 1 from public.businesses where id=p_business for update;
  ...
  if exists(
    ...
    with templates(name,threshold,points_multiplier,perk_note,sort) as (
      values
        ('Member',0::integer,1::numeric,'Standard points earning',0),
        ('Plus',case when p_basis='visits' then 5 else 500 end::integer,
          1.25::numeric,'25% more points',1),
        ('VIP',case when p_basis='visits' then 12 else 1500 end::integer,
