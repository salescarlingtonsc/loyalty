-- NESTLY / PEEKAA V154 — SAFE MULTI-BRANCH FOUNDATION
--
...
create or replace function app.resolve_reporting_branch_scope_v154(
  p_business uuid,
  p_scope_mode text,
...
comment on function app.resolve_reporting_branch_scope_v154(uuid,text,uuid[],uuid) is
  'V154 canonical reporting branch-scope resolver. Current resolves exactly one operational branch; selected resolves a deduplicated non-empty authorised branch set; all resolves only active branches the current membership may see.';

create or replace function app.reporting_scope_label_v154(
  p_business uuid,
  p_scope_mode text,
  p_branch_ids uuid[] default array[]::uuid[],
  p_operational_branch uuid default null::uuid
...
create or replace function public.get_dashboard_summary_v154(
  p_business uuid,
  p_from date,
  p_to date,
...
drop policy if exists promotion_branch_scopes_v154_staff_read
  on public.promotion_branch_scopes_v154;
...
revoke all on public.promotion_branch_scopes_v154 from public,anon,authenticated;
grant select on public.promotion_branch_scopes_v154 to authenticated;

create or replace function app.save_promotion_scope_v154(
  p_business uuid,
  p_promotion uuid,
  p_scope_mode text,
  p_branch_ids uuid[] default array[]::uuid[],
...
begin
  ...
  if v_mode = 'all' then
    delete from public.promotion_branch_scopes_v154 scope
  ...
  insert into public.promotion_branch_scopes_v154(
...
create or replace function app.promotion_available_at_branch_v154(
  p_promotion uuid,
  p_branch uuid
)
returns boolean
...
as $$
  select coalesce((
    select case
      ...
      when exists(
        select 1 from public.promotion_branch_scopes_v154 mapped
...
create or replace function public.assert_promotion_redeemable_at_branch_v154(
  p_business uuid,
  p_promotion uuid,
  p_branch uuid
)
...
begin
  ...
  if not app.promotion_available_at_branch_v154(p_promotion,p_branch) then
...
create or replace function public.business_get_promotion_editor_v154(
  p_business uuid
)
returns jsonb
language plpgsql
      ...
      'branch_scope',case
        when exists(
          ...
          'branch_ids',(
            ...
            from public.promotion_branch_scopes_v154 mapped
...
create or replace function public.business_create_promotion_draft_v154(
  p_business uuid,
  p_promotion_id uuid,
  p_attempt_key uuid,
  p_scope_mode text,
...
create or replace function public.customer_get_promotions_v154(
  p_business uuid,
...
begin
  ...
  with visible as (
    ...
    where content.business_id = p_business
      ...
      and app.promotion_available_at_branch_v154(content.id,p_branch)
...
create or replace function public.preview_campaign_audience_v154(
  p_business uuid,
  p_audience_key text,
  p_scope_mode text default 'all',
  p_branch_ids uuid[] default array[]::uuid[],
...
revoke all on function app.resolve_reporting_branch_scope_v154(uuid,text,uuid[],uuid)
  from public,anon,authenticated;
revoke all on function app.reporting_scope_label_v154(uuid,text,uuid[],uuid)
  from public,anon,authenticated;
revoke all on function app.save_promotion_scope_v154(uuid,uuid,text,uuid[],uuid)
  from public,anon,authenticated;
revoke all on function app.promotion_available_at_branch_v154(uuid,uuid)
  from public,anon,authenticated;
grant execute on function app.resolve_reporting_branch_scope_v154(uuid,text,uuid[],uuid)
  to authenticated;
grant execute on function app.reporting_scope_label_v154(uuid,text,uuid[],uuid)
  to authenticated;
grant execute on function app.promotion_available_at_branch_v154(uuid,uuid)
  to authenticated;

revoke all on function public.get_dashboard_summary_v154(uuid,date,date,text,uuid[],uuid)
  from public,anon,authenticated;
revoke all on function public.staff_list_customers_v154(uuid,text,text,text,uuid[],uuid,integer,integer)
  from public,anon,authenticated;
revoke all on function public.assert_promotion_redeemable_at_branch_v154(uuid,uuid,uuid)
  from public,anon,authenticated;
revoke all on function public.business_get_promotion_editor_v154(uuid)
  from public,anon,authenticated;
revoke all on function public.business_create_promotion_draft_v154(
...
revoke all on function public.customer_get_promotions_v154(uuid,uuid,text)
  from public,anon,authenticated;
revoke all on function public.preview_campaign_audience_v154(uuid,text,text,uuid[],uuid)
  from public,anon,authenticated;

grant execute on function public.get_dashboard_summary_v154(uuid,date,date,text,uuid[],uuid)
  to authenticated;
grant execute on function public.staff_list_customers_v154(uuid,text,text,text,uuid[],uuid,integer,integer)
  to authenticated;
grant execute on function public.assert_promotion_redeemable_at_branch_v154(uuid,uuid,uuid)
  to authenticated;
grant execute on function public.business_get_promotion_editor_v154(uuid)
  to authenticated;
grant execute on function public.business_create_promotion_draft_v154(
  uuid,uuid,uuid,text,uuid[],uuid,text,text,text,text,timestamptz,timestamptz,integer,text,text,text,text
) to authenticated;
grant execute on function public.business_finalize_promotion_v154(
  uuid,uuid,text,uuid[],uuid,text,text,text,text,timestamptz,timestamptz,integer,text,text,text,text,boolean,text,text,integer,integer,text,bigint,bigint,bigint,uuid
) to authenticated;
grant execute on function public.customer_get_promotions_v154(uuid,uuid,text)
  to authenticated;
grant execute on function public.preview_campaign_audience_v154(uuid,text,text,uuid[],uuid)
  to authenticated;
