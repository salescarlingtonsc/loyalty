/* nestly_v656 — A TIER DISCOUNT CAN NAME WHAT IT COVERS, AND A LIMITED ONE CAN ACTUALLY COME OFF
   THE BILL. Owner mark, 2026-08-31 (photo 3):
     "i need the discount to be explained: example 10% off selected products able to select the
      product/services (put all products under description) so when i select that particular item
      it will deduct 10%. if blanket 10% also allow that selection ... also when i 'use' the
      voucher > it must deduct the overall amount by 10%."

   Two defects, one of them silent and expensive:

   (1) A DISCOUNT COULD NOT NAME ANYTHING. tier_benefits_v365 has a product_id column, and its
       own shape CHECK forbids using it on a discount_pct row — so "10% off" always meant 10% off
       the entire bill, and an owner who wanted 10% off haircuts only had no way to say so.
       Scope now lives in its own table, many-to-many, over products AND services. NO ROWS MEANS
       THE WHOLE BILL, which keeps every discount already configured behaving exactly as it does
       today, and matches the convention the branch and reward pickers already use.

   (2) A LIMITED DISCOUNT MOVED NO MONEY. v370 auto-applies unlimited discounts at Record sale and
       deliberately excluded limited ones, on the sound reasoning that spending an allowance has to
       be COUNTED and the only thing that counted was the staff Give button. The gap nobody closed
       is that Give counts and charges nothing: a "20% off, 1 per month" perk was recorded as
       handed over while the customer paid full price. It is now applicable at Record sale, and the
       count is written by record_cart_sale inside the same transaction that creates the sale — so
       the money and the allowance can never disagree, in either direction.

   The pricing authority stays the single authority: the browser still computes no total and no
   discount, it only says WHICH perk the counter chose.

   Rollback suite: db/tests/v656_tier_discount_scope.sql */
begin;

-- ---------------------------------------------------------------------------------------------
-- 1. The scope table. No rows for a benefit = the whole bill.
-- ---------------------------------------------------------------------------------------------
create table if not exists public.tier_benefit_scope_v656(
  ...
  constraint tier_benefit_scope_v656_one_target check (num_nonnulls(product_id, service_id) = 1)
);
create unique index if not exists tier_benefit_scope_v656_product_uk
  on public.tier_benefit_scope_v656(benefit_id, product_id) where product_id is not null;
create unique index if not exists tier_benefit_scope_v656_service_uk
  on public.tier_benefit_scope_v656(benefit_id, service_id) where service_id is not null;
create index if not exists tier_benefit_scope_v656_benefit_idx
  on public.tier_benefit_scope_v656(benefit_id);

alter table public.tier_benefit_scope_v656 enable row level security;

drop policy if exists tier_benefit_scope_v656_read on public.tier_benefit_scope_v656;
create policy tier_benefit_scope_v656_read on public.tier_benefit_scope_v656
  for select using (app.is_super_admin()
                    or app.can_module_read(business_id, 'loyalty')
                    or app.can_module_read(business_id, 'till'));

-- Writes go through business_set_tier_benefits_v365 (SECURITY DEFINER) exactly as the benefit
-- rows themselves do; the table carries no INSERT/UPDATE/DELETE policy, so it is API-unwritable.
revoke all on public.tier_benefit_scope_v656 from public, anon;
grant select on public.tier_benefit_scope_v656 to authenticated;
grant select, insert, update, delete on public.tier_benefit_scope_v656 to service_role;

-- ---------------------------------------------------------------------------------------------
-- 2. Naming helpers.
-- ---------------------------------------------------------------------------------------------
create or replace function app.v656_scope_names(p_benefit uuid)
returns text[]
language sql
stable
set search_path to 'pg_catalog', 'public', 'pg_temp'
as $function$
  select coalesce(array_agg(nm order by nm), '{}'::text[])
    from (
      select p.name as nm from public.tier_benefit_scope_v656 s
        join public.products p on p.id = s.product_id
       where s.benefit_id = p_benefit
      union all
      select sv.name from public.tier_benefit_scope_v656 s
        join public.services sv on sv.id = s.service_id
       where s.benefit_id = p_benefit
    ) q
$function$;

create or replace function app.v656_scope_names_for(p_business uuid, p_products uuid[], p_services uuid[])
returns text[]
language sql
stable
set search_path to 'pg_catalog', 'public', 'pg_temp'
as $function$
  select coalesce(array_agg(nm order by nm), '{}'::text[])
    from (
      select p.name as nm from public.products p
       where p.business_id = p_business and p.id = any(coalesce(p_products, '{}'::uuid[]))
      union all
      select sv.name from public.services sv
       where sv.business_id = p_business and sv.id = any(coalesce(p_services, '{}'::uuid[]))
    ) q
$function$;

/* "10% off Facial, Spa". The 120-character ceiling is tier_benefits_v365.label's own CHECK, so a
   long list collapses to a count rather than failing the owner's save. */
create or replace function app.v656_discount_label(p_discount numeric, p_names text[])
returns text
language sql
immutable
...
as $function$
  select case
    when p_names is null or cardinality(p_names) = 0
      then regexp_replace(trim(to_char(p_discount, 'FM999990.99')), '\.$', '') || '% off'
    when length(regexp_replace(trim(to_char(p_discount, 'FM999990.99')), '\.$', '')
                || '% off ' || array_to_string(p_names, ', ')) <= 120
      then regexp_replace(trim(to_char(p_discount, 'FM999990.99')), '\.$', '')
           || '% off ' || array_to_string(p_names, ', ')
    else regexp_replace(trim(to_char(p_discount, 'FM999990.99')), '\.$', '')
         || '% off ' || cardinality(p_names) || ' selected items'
  end
$function$;

revoke all on function app.v656_scope_names(uuid) from public, anon;
revoke all on function app.v656_scope_names_for(uuid, uuid[], uuid[]) from public, anon;
revoke all on function app.v656_discount_label(numeric, text[]) from public, anon;

-- ---------------------------------------------------------------------------------------------
-- 3. The setter stores the scope.
-- ---------------------------------------------------------------------------------------------
...
begin
  ...
  for v_item in select * from jsonb_array_elements(coalesce(p_benefits,'[]'::jsonb)) loop
    ...
    v_label := case when v_kind='discount_pct' and cardinality(v_scope_names) > 0
      then app.v656_discount_label(v_discount, v_scope_names)
...
-- ---------------------------------------------------------------------------------------------
-- 4. The staff reader carries the covered items.
-- ---------------------------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------------------------
-- 5. The customer's own reader carries them too.
-- ---------------------------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------------------------
-- 6. THE PRICING AUTHORITY.
--    ps1c_plan_checkout and evaluate_checkout each gain ONE defaulted parameter — the perk the
--    counter chose for this sale. The old arities are dropped in the same transaction rather than
--    left beside the new ones: two overloads that a 5-argument call could both satisfy is exactly
--    the ambiguity that took every promotion save down in v410 (PGRST203), and PostgREST resolves
--    a named-argument call against the defaulted parameter perfectly well without them.
-- ---------------------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION app.ps1c_plan_checkout(p_business uuid, p_branch uuid, p_client uuid, p_lines jsonb, p_config uuid, p_tier_benefit uuid DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'app', 'pg_temp'
...
begin
  ...
  if p_client is not null then
    ...
    if v_tier_benefit.id is not null and not v_nonstack_applied then
      ...
      v_tier_scoped := exists(select 1 from public.tier_benefit_scope_v656 sc
                               where sc.benefit_id = v_tier_benefit.id);
      if v_tier_scoped then
        v_tier_base := 0;
        for j in 1 .. coalesce(array_length(v_rem, 1), 0) loop
          if v_rem[j] > 0 and v_id[j] is not null and exists(
            ...
            v_tier_base := v_tier_base + v_rem[j];
          end if;
        end loop;
        -- A bill-level rule discount is not reflected in v_rem, so the scoped base is capped at
        -- what is actually still payable. Without this a bill-level rule plus a scoped tier
        -- discount could together exceed the subtotal.
...
drop function if exists app.ps1c_plan_checkout(uuid, uuid, uuid, jsonb, uuid);
revoke all on function app.ps1c_plan_checkout(uuid, uuid, uuid, jsonb, uuid, uuid) from public, anon;

CREATE OR REPLACE FUNCTION public.evaluate_checkout(p_business uuid, p_branch uuid, p_client uuid, p_lines jsonb, p_idempotency_key uuid, p_tier_benefit uuid DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'app', 'pg_temp'
...
begin
  ...
  v_hash := app.ps1b_sha256(jsonb_build_object(
    ...
    'tier_benefit', p_tier_benefit)::text);
  ...
  v_plan := app.ps1c_plan_checkout(p_business, v_branch, p_client, p_lines, v_config, p_tier_benefit);
...
drop function if exists public.evaluate_checkout(uuid, uuid, uuid, jsonb, uuid);
revoke all on function public.evaluate_checkout(uuid, uuid, uuid, jsonb, uuid, uuid) from public, anon;
grant execute on function public.evaluate_checkout(uuid, uuid, uuid, jsonb, uuid, uuid) to authenticated, service_role;

-- ---------------------------------------------------------------------------------------------
-- 7. Finalising the sale spends the allowance.
-- ---------------------------------------------------------------------------------------------
