/* nestly_v691 — pausing a membership stops its billing clock, and three idempotent RPCs stop
...
comment on column public.memberships.paused_at is
  'nestly_v691: when the current pause began. Non-null exactly while status = ''paused''; the '
...
begin
  ...
  if position('replayed' in v_def) > 0 then
    raise notice 'nestly_v691: enroll_membership_v41 already marks a replay, skipping';
  else
    if (length(v_def) - length(replace(v_def, v_hit, ''))) / nullif(length(v_hit),0) <> 1
      ...
      raise exception 'nestly_v691: an enroll anchor did not match exactly once — body drifted'
        using errcode = 'XX001';
    end if;
    v_new := replace(replace(v_def, v_hit, v_hit_new), v_fresh, v_fresh_new);
    if v_new = v_def then
      raise exception 'nestly_v691: the enroll splice produced no change' using errcode = 'XX001';
    end if;
    execute v_new;
  end if;
end
...
begin
  ...
  if position('jsonb_build_object(''replayed'', true)' in v_def) > 0 then
    raise notice 'nestly_v691: sell_package_v102 already marks a replay, skipping';
  else
    if (length(v_def) - length(replace(v_def, v_sale, ''))) / nullif(length(v_sale),0) <> 1 then
      raise exception 'nestly_v691: the sell_package anchor did not match exactly once'
        using errcode = 'XX001';
    end if;
    v_new := replace(v_def, v_sale, v_sale_new);
    if v_new = v_def then
      raise exception 'nestly_v691: the sell_package splice produced no change' using errcode = 'XX001';
    end if;
    execute v_new;
  end if;
  ...
  if position('jsonb_build_object(''replayed'', true)' in v_def) > 0 then
    raise notice 'nestly_v691: use_package_session_v102 already marks a replay, skipping';
  else
    if (length(v_def) - length(replace(v_def, v_session, ''))) / nullif(length(v_session),0) <> 1 then
      raise exception 'nestly_v691: the use_package_session anchor did not match exactly once'
        using errcode = 'XX001';
    end if;
    v_new := replace(v_def, v_session, v_session_new);
    if v_new = v_def then
      raise exception 'nestly_v691: the use_package_session splice produced no change'
        using errcode = 'XX001';
    end if;
    execute v_new;
  end if;
...
begin
  if not exists (select 1 from information_schema.columns
    ...
    raise exception 'nestly_v691 (F088): memberships has no pause clock' using errcode='XX001';
  end if;
  if position('paused_at = now()' in v_status) = 0
     or position('current_period_end + v_paused_for' in v_status) = 0 then
    raise exception 'nestly_v691 (F088): the pause still leaves the billing clock running'
      using errcode='XX001';
  end if;
  if position('jsonb_build_object(''replayed'', true)' in v_enroll) = 0
    ...
    raise exception 'nestly_v691 (F089): a membership enrollment replay is still indistinguishable'
      using errcode='XX001';
  end if;
  if position('jsonb_build_object(''replayed'', true)' in v_sale) = 0 then
    raise exception 'nestly_v691 (F134): a package sale replay is still indistinguishable'
      using errcode='XX001';
  end if;
  if position('jsonb_build_object(''replayed'', true)' in v_session) = 0 then
    raise exception 'nestly_v691 (F134): a package session replay is still indistinguishable'
      using errcode='XX001';
  end if;
end
$verify$;
