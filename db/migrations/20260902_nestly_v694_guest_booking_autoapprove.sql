/* nestly_v694 — a GUEST booking can auto-approve, instead of failing silently and sitting
   ...
   ('booking_request.auto_approve_skipped_v694', reason 'client_unresolved') and returns null.
...
begin
  ...
  /* nestly_v694: public.appointments.client_id is NOT NULL, and a GUEST request carries no
  ...
  if v_client is null then
    ...
    values(v_req.business_id, null, 'booking_request.auto_approve_skipped_v694',
...
comment on function app.v660_autoapprove_booking_request(uuid) is
  'nestly_v660/v678/v694 decides an unanswered booking request when the firm has auto_approve_changes on. v694: a GUEST request (no customer_client_id) resolves its customer through app.upsert_portal_client + app.apply_booking_consent, the same two calls the manual confirmation makes, because public.appointments.client_id is NOT NULL and the insert used to raise a swallowed 23502. An unresolvable customer is recorded as booking_request.auto_approve_skipped_v694 and left pending, never silently dropped.';
...
begin
  if position('app.upsert_portal_client' in v_def) = 0
    ...
    raise exception 'nestly_v694: auto-approve does not resolve a guest customer the way the manual confirmation does'
      using errcode = 'XX001';
  end if;
  ...
  if position('values(v_req.business_id, v_req.customer_client_id, v_staff' in v_def) <> 0 then
    raise exception 'nestly_v694: the appointment is still inserted with booking_requests.customer_client_id'
      using errcode = 'XX001';
  end if;
  if position('values(v_req.business_id, v_client, v_staff' in v_def) = 0 then
    raise exception 'nestly_v694: the appointment is not inserted with the resolved client'
      using errcode = 'XX001';
  end if;
  if position('auto_approve_skipped_v694' in v_def) = 0 then
    raise exception 'nestly_v694: an unresolvable customer is not recorded — it would be swallowed again'
      using errcode = 'XX001';
  end if;
  ...
  if position('app.upsert_portal_client' in v_def) < position('if v_staff is null then return null; end if;' in v_def) then
    raise exception 'nestly_v694: the customer is resolved before auto-approve has finished refusing'
      using errcode = 'XX001';
  end if;
  if exists (select 1 from information_schema.routine_privileges
    ...
    raise exception 'nestly_v694: the auto-approve helper became reachable by a client role'
      using errcode = 'XX001';
  end if;
  ...
  if exists (select 1 from information_schema.columns
    ...
    raise exception 'nestly_v694: appointments.client_id is no longer NOT NULL — re-read this migration'
      using errcode = 'XX001';
  end if;
end
$verify$;
