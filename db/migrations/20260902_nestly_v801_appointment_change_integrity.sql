/* nestly_v801 — four holes in the appointment/booking change path.
...
begin
  ...
  if position('kept_booked' in v_def) > 0 then
    raise notice 'nestly_v801: v508 already files a reschedule request, skipping';
  else
    if (length(v_def) - length(replace(v_def, v_decl, ''))) / nullif(length(v_decl),0) <> 1
      ...
      raise exception 'nestly_v801: a v508 anchor did not match exactly once — the body drifted'
        using errcode = 'XX001';
    end if;
    v_new := replace(replace(replace(v_def, v_decl, v_decl_new), v_read, v_read_new),
                     v_arm, v_arm_new);
    if v_new = v_def then
      raise exception 'nestly_v801: the v508 splice produced no change' using errcode = 'XX001';
    end if;
    execute v_new;
  end if;
end
...
begin
  ...
  if position('branch_breaks' in v_def) > 0 then
    raise notice 'nestly_v801: the slot lister already honours branch breaks, skipping';
  else
    if (length(v_def) - length(replace(v_def, v_declare, ''))) / nullif(length(v_declare),0) <> 1
      ...
      raise exception 'nestly_v801: a slot-lister anchor did not match exactly once — body drifted'
        using errcode = 'XX001';
    end if;
    ...
    if v_new = v_def then
      raise exception 'nestly_v801: the slot-lister splice produced no change' using errcode = 'XX001';
    end if;
    execute v_new;
  end if;
end
...
begin
  ...
  if position('and not app.staff_free_for_appointment_v47' in v_def) > 0 then
    raise notice 'nestly_v801: decide_change already skips the staff guard, skipping';
  else
    if (length(v_def) - length(replace(v_def, v_anchor, ''))) / nullif(length(v_anchor),0) <> 1 then
      raise exception 'nestly_v801: the decide_change anchor did not match exactly once'
        using errcode = 'XX001';
    end if;
    v_new := replace(v_def, v_anchor, v_inject);
    if v_new = v_def then
      raise exception 'nestly_v801: the decide_change splice produced no change' using errcode = 'XX001';
    end if;
    execute v_new;
  end if;
end
...
begin
  if position('kept_booked' in v_v508) = 0
    ...
    raise exception 'nestly_v801 (F064): v508 still cancels without asking the business'
      using errcode = 'XX001';
  end if;
  if position('branch_breaks' in v_lister) = 0
    ...
    raise exception 'nestly_v801 (F065): the slot lister still disagrees with the write guard'
      using errcode = 'XX001';
  end if;
  if position('and not app.staff_free_for_appointment_v47' in v_decide) = 0 then
    raise exception 'nestly_v801 (F066): decide_change still refuses staff-less appointments'
      using errcode = 'XX001';
  end if;
  ...
  if v_trigger is null or position('UPDATE OF' in upper(v_trigger)) = 0 then
    raise exception 'nestly_v801 (F067): auto-approve still fires on INSERT only'
      using errcode = 'XX001';
  end if;
end
$verify$;
