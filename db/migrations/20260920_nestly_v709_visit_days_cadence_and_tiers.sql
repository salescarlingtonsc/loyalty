...
-- ---------------------------------------------------------------------------------------------
-- 2 · app.tier_resolve_v426 — the visits-basis metric moves from a sale count to a distinct
--     visit-day count, the same authority (a) above now uses.
--     Merged 2026-09-03: main's nestly_v677 ("a reversed sale is not a visit") had already moved
--     this line onto app.client_qualifying_visits_v677, which nets reversals. The two rulings
--     compose — a visit is a distinct Singapore day among the NON-REVERSED qualifying sales — so
--     the anchor is v677's line (comment-free, as production stores it) and the replacement keeps
--     v677's reversal netting verbatim while counting distinct visit days over that set.
-- ---------------------------------------------------------------------------------------------
do $patch_tier$
declare
  v_def text;
  v_anchor constant text :=
E'    v_metric := app.client_qualifying_visits_v677(p_business, p_client);';
  v_new constant text :=
E'    with visit_rows as (
      select s.id, s.reversal_of, s.occurred_at
        from public.sales s
       where s.business_id = p_business and s.client_id = p_client and s.counts_as_visit
    )
    select count(distinct app.ci_visit_day_v699(v.occurred_at)) into v_metric
      from visit_rows v
