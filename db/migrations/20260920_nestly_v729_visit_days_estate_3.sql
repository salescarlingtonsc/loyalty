...
declare
  v_def text;
  -- Merged 2026-09-03: main's nestly_v677 had already moved this line onto
  -- app.client_qualifying_visits_v677 (a reversed sale is not a visit). The two rulings compose:
  -- distinct Singapore visit days over the NON-REVERSED qualifying sales. The anchor is v677's
  -- line as production stores it (comment-free); the replacement keeps v677's netting verbatim.
  v_anchor constant text :=
E'    v_metric := app.client_qualifying_visits_v677(p_business, v_client);';
  v_new constant text :=
E'    with visit_rows as (
      select s.id, s.reversal_of, s.occurred_at
        from public.sales s
       where s.business_id=p_business and s.client_id=v_client and s.counts_as_visit
    )
