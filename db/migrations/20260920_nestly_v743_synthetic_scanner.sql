...
-- =============================================================================================
-- Merged 2026-09-03: this block used to restate the whole body as captured BEFORE main's
-- nestly_v685 (Singapore-day authority) patched app.issue_bringback_for_business_v361. Restating that capture would
-- silently undo v685. The exclusion hunk is now applied to the LIVE body inside the check
-- block below (capture -> assert the anchor occurs exactly once -> replace -> execute), so
-- v685's semantics and every later patch survive, and the round-trip assertion that follows
-- still proves nothing but the exclusion moved.
do $check11$
declare v_before text; v_after text; v_expect text;
...
begin
  ...
  if (length(v_before) - length(replace(v_before, v_old1, ''))) / length(v_old1) <> 1 then
    raise exception 'app.issue_bringback_for_business_v361: exclusion anchor must occur exactly once in the live body';
  end if;
  execute replace(v_before, v_old1, v_new1);
  select pg_get_functiondef(p.oid) into v_after
    from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'app' and p.proname = 'issue_bringback_for_business_v361';
  if position(v_old1 in v_before) = 0 then
...
insert into app.ci_synthetic_scan_allowlist_v743 (function_signature, reason, added_by_migration) values
  ('app.client_qualifying_visits_v677(p_business uuid, p_client uuid)', 'single-client visit kernel (p_client) from main''s nestly_v677 (a reversed sale is not a visit), merged 2026-09-03 — one customer''s own non-reversed visit count, the same shape as app.client_points_balance_v409 below; synthetic exclusion belongs to the caller presenting a population, never to a per-client kernel.', 'nestly_v743'),
