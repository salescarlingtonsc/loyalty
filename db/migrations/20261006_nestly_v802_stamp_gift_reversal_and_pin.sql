/* nestly_v802 — a stamp gift can be un-redeemed, and the wallet home card reads the card the
...
begin
  ...
  if position('v802_stamp_reversal_redemption_id' in v_def) > 0 then
    ...
    if (length(v_def) - length(replace(v_def, v_anchor, ''))) / nullif(length(v_anchor),0) <> 1
      ...
      raise exception 'nestly_v802: a reversal anchor did not match exactly once — body drifted'
        using errcode = 'XX001';
    end if;
    v_new := replace(replace(v_def, v_decl, v_decl_new), v_anchor, v_inject);
    if v_new = v_def then
      raise exception 'nestly_v802: the reversal splice produced no change' using errcode = 'XX001';
    end if;
    execute v_new;
  end if;
end
...
begin
  ...
  if position('v802_config_provenance_ok' in v_def) > 0 then
    ...
    if (length(v_def) - length(replace(v_def, v_base, ''))) / nullif(length(v_base),0) <> 1 then
      raise exception 'nestly_v802: the base config anchor did not match exactly once'
        using errcode = 'XX001';
    end if;
    v_new := replace(v_def, v_base, v_base_new);
    if v_new = v_def then
      raise exception 'nestly_v802: the base config splice produced no change' using errcode = 'XX001';
    end if;
    execute v_new;
  end if;
  ...
  if position('v802_config_provenance_ok' in v_def) > 0 then
    ...
    if (length(v_def) - length(replace(v_def, v_wrap, ''))) / nullif(length(v_wrap),0) <> 1 then
      raise exception 'nestly_v802: the wrapper config anchor did not match exactly once'
        using errcode = 'XX001';
    end if;
    v_new := replace(v_def, v_wrap, v_wrap_new);
    if v_new = v_def then
      raise exception 'nestly_v802: the wrapper config splice produced no change' using errcode = 'XX001';
    end if;
    execute v_new;
  end if;
end
...
begin
  ...
  if position('stamp_ok' in v_def) > 0 then
    raise notice 'nestly_v802: the reversal workflow reader already knows stamps, skipping';
  else
    if (length(v_def) - length(replace(v_def, v_lateral, ''))) / nullif(length(v_lateral),0) <> 1
      ...
      raise exception 'nestly_v802: a workflow-reader anchor did not match the expected count'
        using errcode = 'XX001';
    end if;
    ...
    if v_new = v_def then
      raise exception 'nestly_v802: the workflow-reader splice produced no change' using errcode = 'XX001';
    end if;
    execute v_new;
  end if;
end
...
begin
  ...
  if position('stamp_cycle_version_v416' in v_def) > 0 then
    raise notice 'nestly_v802: the wallet card already follows the pinned version, skipping';
  else
    if (length(v_def) - length(replace(v_def, v_anchor, ''))) / nullif(length(v_anchor),0) <> 1 then
      raise exception 'nestly_v802: the wallet-card anchor did not match exactly once'
        using errcode = 'XX001';
    end if;
    v_new := replace(v_def, v_anchor, v_inject);
    if v_new = v_def then
      raise exception 'nestly_v802: the wallet-card splice produced no change' using errcode = 'XX001';
    end if;
    execute v_new;
  end if;
end
...
begin
  if position('append-only' in v_guard) = 0
    ...
    raise exception 'nestly_v802: the stamp evidence guard is not append-only-by-default'
      using errcode = 'XX001';
  end if;
  ...
  if exists (select 1 from pg_trigger t
             ...
             where not t.tgisinternal
               ...
               and t.tgrelid in ('public.stamp_milestone_claims'::regclass,
                                 'public.stamp_cycles'::regclass)) then
    raise exception 'nestly_v802: a stamp table is still on the shared v34 guard as well'
      using errcode = 'XX001';
  end if;
  if not exists (select 1 from pg_trigger t
    ...
    raise exception 'nestly_v802: the shared v34 guard was disturbed on another table'
      using errcode = 'XX001';
  end if;
  ...
  if position('stamp_cycle_version_v416' in v_wallet) = 0 then
    raise exception 'nestly_v802 (F128): the wallet card still reads the active version'
      using errcode = 'XX001';
  end if;
end
$verify$;
