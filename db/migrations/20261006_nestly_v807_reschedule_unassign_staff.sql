/* nestly_v807 — "Anyone available" can actually un-assign a booking request when staff move it.
...
begin
  ...
  /* nestly_v807: naming a team member AND asking to clear the choice is a contradiction. It is
  ...
  /* nestly_v807: NULL means "unchanged" here and always has, so "leave it open" needs a word of
...
comment on function public.staff_reschedule_and_confirm_booking_request_v329(uuid, uuid, timestamptz, uuid, boolean) is
  'nestly_v329/v807 moves a pending booking request to a new time (and optionally a new team member) and confirms it through staff_decide_booking_request_v73. v807: p_clear_staff = true un-assigns the request, because p_staff = NULL has always meant "unchanged" and there was therefore no way to honour the customer-facing "Anyone available" choice. The two are mutually exclusive and passing both is refused.';
...
begin
  ...
  if v_count <> 1 then
    raise exception 'nestly_v807: % overloads of the reschedule RPC exist — PostgREST would answer a named-argument call with PGRST203', v_count
      using errcode = 'XX001';
  end if;
  ...
  if position('p_clear_staff boolean DEFAULT false' in v_def) = 0 then
    raise exception 'nestly_v807: the clear flag is not defaulted, so the four-argument call is no longer valid'
      using errcode = 'XX001';
  end if;
  if position('when coalesce(p_clear_staff, false) then null' in v_def) = 0 then
    raise exception 'nestly_v807: the reschedule RPC still cannot clear an existing staff assignment'
      using errcode = 'XX001';
  end if;
  if position('else coalesce(p_staff, staff_id)' in v_def) = 0 then
    raise exception 'nestly_v807: the unchanged-when-not-clearing behaviour was lost'
      using errcode = 'XX001';
  end if;
  if position('choose a team member or leave it open, not both' in v_def) = 0 then
    raise exception 'nestly_v807: naming a member while clearing is no longer refused'
      using errcode = 'XX001';
  end if;
  if position('app.can_module_write(p_business, ''bookings'')' in v_def) = 0 then
    raise exception 'nestly_v807: the bookings write guard was lost' using errcode = 'XX001';
  end if;
  if exists (select 1 from information_schema.routine_privileges
              where routine_schema = 'public'
                and routine_name = 'staff_reschedule_and_confirm_booking_request_v329'
                and grantee in ('anon','PUBLIC')) then
    raise exception 'nestly_v807: the reschedule RPC became anonymously reachable' using errcode = 'XX001';
  end if;
  if not exists (select 1 from information_schema.routine_privileges
                  where routine_schema = 'public'
                    and routine_name = 'staff_reschedule_and_confirm_booking_request_v329'
                    and grantee = 'authenticated') then
    raise exception 'nestly_v807: the browser lost execute on the reschedule RPC' using errcode = 'XX001';
  end if;
end
$verify$;
