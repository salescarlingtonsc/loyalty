-- Rollback-only nestly_v801 acceptance: four holes in the appointment/booking change path.
--
-- WHAT THE BUGS WERE
...
$$;
grant execute on function pg_temp.as_v801_system() to public;

create or replace function pg_temp.as_v801_user(p_uid uuid, p_role text default 'authenticated')
...
$$;
grant execute on function pg_temp.as_v801_user(uuid,text) to public;
...
begin
  ...
  values ('00000000-0000-0000-0000-000000000000',p_owner,'authenticated','authenticated',
          'v801-owner-'||substr(p_owner::text,1,8)||'@example.test','',now(),now(),now()),
         ('00000000-0000-0000-0000-000000000000',p_customer_user,'authenticated','authenticated',
          'v801-cust-'||substr(p_customer_user::text,1,8)||'@example.test','',now(),now(),now())
  ...
  values (p_business,'V689 '||p_label,'v801-'||substr(p_business::text,1,8),
          'retail','SGD',
  ...
  update public.business_workspace_controls_v94
     set approval_status='approved', decided_by=p_owner, decided_at=now(),
         decision_reason='v801 rollback fixture'
...
begin
  ...
  perform pg_temp.as_v801_user(uManual);
  v_err := null;
  begin
  ...
  begin
    ...
    perform pg_temp.as_v801_user(uAuto);
    v_err := null;
    begin
      select public.customer_reschedule_appointment_v508(
  ...
  perform pg_temp.as_v801_user(oManual);
  v_err := null;
  begin
  ...
  begin
    ...
    perform pg_temp.as_v801_user(uList);
