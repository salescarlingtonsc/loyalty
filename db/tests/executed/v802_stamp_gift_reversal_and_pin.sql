-- Rollback-only nestly_v802 acceptance: a stamp gift can be un-redeemed, and the wallet home
...
--       reason is given (before v802 it was false with the points-ledger refusal).
...
$$;
grant execute on function pg_temp.as_v802_system() to public;

create or replace function pg_temp.as_v802_user(p_uid uuid, p_role text default 'authenticated')
...
$$;
grant execute on function pg_temp.as_v802_user(uuid,text) to public;
...
begin
  ...
  values ('00000000-0000-0000-0000-000000000000',p_owner,'authenticated','authenticated',
          'v802-owner-'||substr(p_owner::text,1,8)||'@example.test','',now(),now(),now())
  on conflict (id) do nothing;

  perform set_config('app.v79_system_transition','on',true);
  insert into public.businesses(id,name,slug,industry,currency,enabled_modules,points_mode)
  values (p_business,'V690 '||p_label,'v802-'||substr(p_business::text,1,8),
          'fnb','SGD',
  ...
  update public.business_workspace_controls_v94
     set approval_status='approved', version=version+1, decided_by=p_owner,
         decided_at=clock_timestamp(), decision_reason='v802 rollback fixture',
...
begin
  ...
  values (v_seed,bS,clientS,'adjust',5,'v802 seed stamps',null,spineS,now()-interval '1 day');
  ...
  values (v_seed,bP,clientP,'adjust',50,'v802 seed points',null,spineP);
  perform set_config('app.points_ledger_insert_id','',true);
  perform set_config('app.points_ledger_write_scope','',true);
  ...
  v_card := app.c45_base_actionable_wallet_card(
    bS,clientS,'v802-'||substr(bS::text,1,8),'V690 Kopi','fnb','SGD',
  ...
  perform pg_temp.as_v802_user(oS);
  v_res := public.redeem_reward_at_context(
    bS,clientS,giftS,'v802-stamp-claim-'||replace(gen_random_uuid()::text,'-',''),
    brS,null,null)::jsonb;
  ...
  perform pg_temp.as_v802_user(oS);
  v_key := 'v802-stamp-reverse-'||replace(gen_random_uuid()::text,'-','');
  v_err := null;
  begin
    v_res := public.reverse_loyalty_redemption(
      bS,v_redemption,'v802 acceptance: given to the wrong customer',v_key)::jsonb;
  ...
  begin
    v_res := public.redeem_reward_at_context(
      bS,clientS,giftS,'v802-stamp-reclaim-'||replace(gen_random_uuid()::text,'-',''),
      brS,null,null)::jsonb;
  exception when others then
    v_err := sqlstate; v_msg := sqlerrm; v_res := null;
  end;
  ...
  perform pg_temp.as_v802_user(oP);
  v_err := null; v_msg := null;
  begin
    v_res := public.redeem_reward_at_context(
      bP,clientP,giftP,'v802-points-claim-'||replace(gen_random_uuid()::text,'-',''),
      brP,null,null)::jsonb;
  exception when others then
    v_err := sqlstate; v_msg := sqlerrm; v_res := null;
  end;
  ...
  begin
    v_res := public.reverse_loyalty_redemption(
      bP,v_redemption2,'v802 acceptance: points control reversal',
      'v802-points-reverse-'||replace(gen_random_uuid()::text,'-',''))::jsonb;
  exception when others then
    v_err := sqlstate; v_msg := sqlerrm; v_res := null;
  end;
  perform pg_temp.as_v802_system();
