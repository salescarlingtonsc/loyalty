-- Rollback-only nestly_v803 acceptance: pausing a membership stops its billing clock, and three
...
$$;
grant execute on function pg_temp.as_v803_system() to public;

create or replace function pg_temp.as_v803_user(p_uid uuid, p_role text default 'authenticated')
...
$$;
grant execute on function pg_temp.as_v803_user(uuid,text) to public;
...
begin
  ...
  values ('00000000-0000-0000-0000-000000000000',p_owner,'authenticated','authenticated',
          'v803-owner-'||substr(p_owner::text,1,8)||'@example.test','',now(),now(),now())
  on conflict (id) do nothing;

  perform set_config('app.v79_system_transition','on',true);
  insert into public.businesses(id,name,slug,industry,currency,enabled_modules)
  values (p_business,'V691 '||p_label,'v803-'||substr(p_business::text,1,8),
          'fnb','SGD',
  ...
  update public.business_workspace_controls_v94
     set approval_status='approved', version=version+1, decided_by=p_owner,
         decided_at=clock_timestamp(), decision_reason='v803 rollback fixture',
...
declare
  ...
  cLegacy uuid := gen_random_uuid();     -- paused before v803 existed (no paused_at)
...
begin
  ...
  perform pg_temp.as_v803_user(oA);
  v_err := null;
  begin
  ...
  perform pg_temp.as_v803_system();
  select * into v_row from public.memberships where id=mLegacy;
  perform pg_temp.as_v803_user(oB);
  v_err := null;
  begin
  ...
  perform pg_temp.as_v803_user(oA);
  v_key_text := 'v803-session-'||replace(gen_random_uuid()::text,'-','');
  v_err := null;
  begin
