-- Rollback-only nestly_v805 acceptance: deleting a stamp gift withdraws it from the NEXT card,
...
begin
  ...
  values ('00000000-0000-0000-0000-000000000000',v_owner,'authenticated','authenticated',
          'zz-v805-owner-'||substr(v_owner::text,1,8)||'@example.test','',now(),now(),now()),
         ('00000000-0000-0000-0000-000000000000',v_cust,'authenticated','authenticated',
          'zz-v805-cust-'||substr(v_cust::text,1,8)||'@example.test','',now(),now(),now());

  perform set_config('app.v79_system_transition','on',true);
  insert into public.businesses(id,name,slug,enabled_modules,points_mode)
  values (v_biz,'V693 Stamp Kopi','zz-v805-'||substr(v_biz::text,1,8),array['loyalty'],'redeem');
  ...
  update public.business_workspace_controls_v94
     set approval_status='approved',version=version+1,decided_by=v_owner,
         decided_at=clock_timestamp(),decision_reason='v805 acceptance fixture',
  ...
  values (v_seed,v_biz,v_client,'adjust',4,'v805 seed stamps',v_owner,v_spine,now()-interval '1 day');
  ...
  -- ==========================================================================================
  v_res := app.redeem_reward_core(v_biz,v_client,v_free,'v805-pinned-claim-01',v_branch)::jsonb;
  ...
  insert into public.points_ledger(id,business_id,client_id,entry_type,points,reference,actor,
                                   programme_id,created_at)
  values (v_seed2,v_biz,v_client2,'adjust',4,'v805 new card stamps',v_owner,v_spine,now());
  perform set_config('app.points_ledger_insert_id','',true);
  perform set_config('app.points_ledger_write_scope','',true);
  insert into public.points_batches(business_id,client_id,programme_id,earned,remaining)
  ...
  begin
    perform app.redeem_reward_core(v_biz,v_client2,v_free,'v805-newcard-claim-01',v_branch);
  ...
  raise notice 'v805 ok: 9/9 — a deleted stamp gift is withdrawn from the next card only';
end
$v805$;
