-- Rollback-only nestly_v806 acceptance: a GUEST booking auto-approves, reusing the customer the
...
--      appointment. This is the assertion that fails on the pre-v806 function.
...
begin
  ...
  update public.business_workspace_controls_v94
     set approval_status = 'approved', decided_at = now(), decision_reason = 'v806 fixture'
  ...
  insert into auth.users(id, email) values (v_bound_user, 'v806.bound@example.com');
  insert into public.clients(id, business_id, full_name, phone, email)
  values (v_bound_client, v_business, 'V694 Bound', '+6581000001', 'v806.bound@example.com');
  ...
  insert into public.clients(id, business_id, full_name, phone, email)
  values (v_known_client, v_business, 'V694 Known Already', '+6581000005', 'v806.known@example.com');

  -- ------------------------------------------------------------------ 1. positive control
  v_res := public.internal_public_booking_submit(
    v_slug, 'V694 Bound', 'v806.bound@example.com', '+6581000001', v_service,
    1, (v_day + time '09:00') at time zone 'Asia/Singapore', null, null, false,
    encode(sha256(convert_to('v806:t1:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v806:i1:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v806:f1:' || v_business::text, 'UTF8')), 'hex'),
  ...
  -- ------------------------------------------------------------------ 2. the guest
  v_res := public.internal_public_booking_submit(
    v_slug, 'V694 Guest', 'v806.guest@example.com', '+6581000694', v_service,
    1, (v_day + time '11:00') at time zone 'Asia/Singapore', null, null, true,
    encode(sha256(convert_to('v806:t2:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v806:i2:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v806:f2:' || v_business::text, 'UTF8')), 'hex'),
  ...
  v_res := public.internal_public_booking_submit(
    v_slug, 'V694 Guest Again', 'v806.guest.alt@example.com', '+6581000694', v_service,
    1, (v_day + time '13:00') at time zone 'Asia/Singapore', null, null, false,
    encode(sha256(convert_to('v806:t4:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v806:i4:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v806:f4:' || v_business::text, 'UTF8')), 'hex'),
    null, v_staff, v_branch);
  ...
  v_res := public.internal_public_booking_submit(
    v_slug, 'V694 Known Already', 'v806.known@example.com', '+6581000005', v_service,
    1, (v_day + time '15:00') at time zone 'Asia/Singapore', null, null, false,
    encode(sha256(convert_to('v806:t5:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v806:i5:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v806:f5:' || v_business::text, 'UTF8')), 'hex'),
    null, v_staff, v_branch);
  select appointment.client_id into v_client_again
    from public.appointments appointment
    join public.booking_requests request_row on request_row.appointment_id = appointment.id
  ...
  v_res := public.internal_public_booking_submit(
    v_slug, 'V694 Nobody Free', 'v806.nobody@example.com', '+6581000006', v_service,
    1, (v_day + time '17:00') at time zone 'Asia/Singapore', null, null, true,
    encode(sha256(convert_to('v806:t6:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v806:i6:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v806:f6:' || v_business::text, 'UTF8')), 'hex'),
    null, null, v_branch);
