-- Rollback-only acceptance for LOYALTY-CATALOGUE-SOURCE-001 and LOYALTY-TIER-STARTER-001.
-- Run after the canonical chain through v148 in a disposable local database.
begin;
\ir fixtures/pristine_chain_fixture.psql

create or replace function pg_temp.as_v148_user(p_uid uuid)
...
$$;
grant execute on function pg_temp.as_v148_user(uuid) to public;

do $v148_owner_launch_closure$
declare
  v_business uuid;
  v_owner uuid;
  v_outsider uuid;
...
begin
  ...
  if v_business is null or v_owner is null or v_outsider is null or v_client is null then
    raise exception 'v148 pristine fixture identities are missing';
  end if;
  ...
  insert into auth.users(
    ...
    'authenticated','authenticated','v148-customer-'||v_customer::text||'@example.test',
    '+6580000148','',now(),now(),now(),now()
  );
  ...
  perform pg_temp.as_v148_user(v_owner);
  ...
  if (select array_agg(name order by sort)
      from public.loyalty_tier_versions
      where config_version_id=v_defaults_draft)
      is distinct from array['Gold','Platinum','Diamond']::text[] then
    raise exception 'recommended tier names were not Gold, Platinum and Diamond';
  end if;
  ...
  reset role;
  perform pg_temp.as_v148_user(v_outsider);
  begin
    perform public.save_loyalty_tier_draft_v143(v_draft,jsonb_build_object(
  ...
  reset role;
  perform pg_temp.as_v148_user(v_customer);
