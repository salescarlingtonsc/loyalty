-- Rollback-only v649 acceptance suite: the single BEFORE INSERT trigger on sale_items
-- (app.sale_items_category_stamp_v649) stamps a service line through service_canonical_map
-- at write time, and does NOT stamp a custom line (no approved node names that question yet).
-- Depends on the v648 mapping already existing for the fixture service (facial.hydration).
-- Run after the complete canonical chain through v649 in a disposable database (or as a
...
begin
  ...
  -- pre-existing mapping this suite depends on (established by v648's flow in production;
  -- reproduced here as a fixture row rather than re-running the full v648 owner flow).
...
-- ---------------------------------------------------------------------------
-- E. v649 sale_items category stamping
-- ---------------------------------------------------------------------------
do $e$
declare
...
begin
  ...
  v_res := public.record_quick_sale(
    ...
    p_client=>v_client,
    p_note=>'glass skin facial visit', p_idempotency_key=>'phasebc-qs-v649-000001')::jsonb;
  v_sale := (v_res #>> '{sale,id}')::uuid;

  perform pg_temp.as_postgres();
  -- the quick-sale custom line must NOT be stamped
  select * into v_line from public.sale_items where sale_id=v_sale and item_type='custom';
  if v_line.canonical_node_key is not null then
    raise exception 'E1: custom line must not carry a category stamp';
  end if;

  -- a service line resolves through the mapping and is stamped
  ...
  if v_line.canonical_node_key <> 'facial.hydration' or v_line.taxonomy_version_no <> 1 then
    raise exception 'E2: service line not stamped: % / %', v_line.canonical_node_key, v_line.taxonomy_version_no;
  end if;

  raise notice 'E OK: category snapshot at sale';
end
$e$;
