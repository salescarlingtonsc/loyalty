-- Rollback-only nestly_v696 acceptance: a customer with no registration-preferences row can save
...
--   2. That customer CAN now save the choice — this is the assertion that fails pre-v696.
...
create or replace function pg_temp.as_v696_system() returns void language plpgsql as $$
begin
  execute 'reset role';
...
create or replace function pg_temp.as_v696_user(p_uid uuid) returns void language plpgsql as $$
begin
  execute 'reset role';
  perform set_config('request.jwt.claim.sub','',true);
  perform set_config('request.jwt.claims','{}',true);
...
begin
  ...
  values (p_uid, 'v696-' || substr(p_uid::text, 1, 8) || '@example.test');
...
begin
  perform pg_temp.as_v696_system();
  ...
  perform pg_temp.as_v696_user(uA);
  v_err := null;
  begin
    select public.customer_set_platform_marketing_preference(true, 'v696-first-save-a') into v_res;
  ...
  select * into v_event from public.customer_platform_marketing_consent_events e
   where e.identity_id = iA and e.idempotency_key = 'v696-first-save-a';
  ...
  perform pg_temp.as_v696_user(uB);
  v_err := null;
  begin
    select public.customer_set_platform_marketing_preference(false, 'v696-first-save-b') into v_res;
  exception when others then v_err := sqlstate; v_res := null;
  end;
  v_read := public.customer_get_platform_marketing_preference();
  perform pg_temp.as_v696_system();
  ...
  v_err := null;
  begin
    select public.customer_set_platform_marketing_preference(false, 'v696-second-save-a') into v_res;
  exception when others then v_err := sqlstate; v_res := null;
  end;
  perform pg_temp.as_v696_system();
  select count(*) into v_count
  ...
  perform pg_temp.as_v696_user(uC);
  v_err := null;
  begin
    perform public.customer_set_platform_marketing_preference(true, 'v696-idem-c');
    select public.customer_set_platform_marketing_preference(true, 'v696-idem-c') into v_res;
  exception when others then v_err := sqlstate; v_res := null;
  end;
  if v_err is null and coalesce((v_res->>'opted_in')::boolean, false) then
    v_err := null;
    begin
      perform public.customer_set_platform_marketing_preference(false, 'v696-idem-c');
  ...
  v_err := null;
  begin
    perform public.customer_set_platform_marketing_preference(true, 'v696-anon-attempt');
  exception when others then v_err := sqlstate;
  end;
  select count(*) into v_count from public.customer_platform_marketing_consent_events e
   where e.idempotency_key = 'v696-anon-attempt';
...
begin
  ...
  if v_all <> 8 then
    raise exception 'nestly_v696: % of 8 assertions ran — the suite aborted early', v_all;
  end if;
  if v_bad > 0 then
    raise exception 'nestly_v696: % assertion(s) FAILED: %', v_bad,
