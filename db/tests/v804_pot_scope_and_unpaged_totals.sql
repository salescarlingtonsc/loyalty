-- Rollback-only nestly_v804 acceptance: two readers that answered with the wrong rows.
--
-- WHAT THE BUGS WERE
...
$$;
grant execute on function pg_temp.as_v804_system() to public;

create or replace function pg_temp.as_v804_user(p_uid uuid, p_role text default 'authenticated')
...
$$;
grant execute on function pg_temp.as_v804_user(uuid,text) to public;
...
create or replace function pg_temp.as_v804_platform(p_uid uuid)
returns void language plpgsql as $$
begin
  execute 'reset role';
...
begin
  ...
  values ('00000000-0000-0000-0000-000000000000',p_owner,'authenticated','authenticated',
          'v804-owner-'||substr(p_owner::text,1,8)||'@example.test','',now(),now(),now())
  on conflict (id) do nothing;

  perform set_config('app.v79_system_transition','on',true);
  insert into public.businesses(id,name,slug,industry,currency,enabled_modules,points_mode)
  values (p_business,'V692 '||p_label,'v804-'||substr(p_business::text,1,8),
          'fnb','SGD',
  ...
  update public.business_workspace_controls_v94
     set approval_status='approved', version=version+1, decided_by=p_owner,
         decided_at=clock_timestamp(), decision_reason='v804 rollback fixture',
...
begin
  ...
  values (v_id,p_business,p_client,'adjust',p_points,'v804 seed',null,p_programme);
...
begin
  ...
  -- ------------------------------------------------------------------ 1. the directory
  perform pg_temp.as_v804_user(oA);
  ...
  values ('00000000-0000-0000-0000-000000000000',admin,'authenticated','authenticated',
          'v804-admin-'||substr(admin::text,1,8)||'@example.test','',now(),now(),now());
  insert into public.super_admins(user_id,email,note)
  values (admin,'v804-admin-'||substr(admin::text,1,8)||'@example.test',
          'synthetic rolled-back v804 proof');
  ...
  perform pg_temp.as_v804_platform(admin);
  ...
  perform pg_temp.as_v804_user(oRep);
  v_err := null;
  begin
