-- Rollback-only nestly_v807 acceptance: "Anyone available" on a staff reschedule really does
...
$$;
grant execute on function pg_temp.as_v807_system() to authenticated;

create or replace function pg_temp.as_v807_user(p_uid uuid) returns void language plpgsql as $$
begin
  execute 'reset role';
  perform set_config('request.jwt.claim.sub','',true);
  perform set_config('request.jwt.claims','{}',true);
...
begin
  -- ----------------------------------------------------------------------------- FIXTURE
  perform pg_temp.as_v807_system();
  insert into auth.users(id, email) values (v_owner, 'v807.owner@example.com');
  insert into auth.users(id, email) values (v_outsider, 'v807.outsider@example.com');
  ...
  update public.business_workspace_controls_v94
     set approval_status = 'approved', decided_at = now(), decision_reason = 'v807 fixture'
  ...
  -- round-robin and lands on somebody who IS free. Two observable differences, one cause: pre-v807
  ...
  v_res := public.internal_public_booking_submit(
    v_slug, 'V695 Guest A', 'v807.a@example.com', '+6581000201', v_service,
    1, (v_day + time '09:00') at time zone 'Asia/Singapore', null, null, false,
    encode(sha256(convert_to('v807:t2:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v807:i2:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v807:f2:' || v_business::text, 'UTF8')), 'hex'),
  ...
  perform pg_temp.as_v807_user(v_owner);
  ...
  v_res := public.internal_public_booking_submit(
    v_slug, 'V695 Guest B', 'v807.b@example.com', '+6581000202', v_service,
    1, (v_day + time '12:00') at time zone 'Asia/Singapore', null, null, false,
    encode(sha256(convert_to('v807:t3:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v807:i3:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v807:f3:' || v_business::text, 'UTF8')), 'hex'),
    null, v_staff, v_branch);
  v_request := nullif(v_res->>'request_id','')::uuid;
  perform pg_temp.as_v807_user(v_owner);
  ...
  v_res := public.internal_public_booking_submit(
    v_slug, 'V695 Guest C', 'v807.c@example.com', '+6581000203', v_service,
    1, (v_day + time '14:00') at time zone 'Asia/Singapore', null, null, false,
    encode(sha256(convert_to('v807:t4:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v807:i4:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v807:f4:' || v_business::text, 'UTF8')), 'hex'),
    null, v_staff, v_branch);
  v_request := nullif(v_res->>'request_id','')::uuid;
  perform pg_temp.as_v807_user(v_owner);
  v_res := public.staff_reschedule_and_confirm_booking_request_v329(
  ...
  v_res := public.internal_public_booking_submit(
    v_slug, 'V695 Guest D', 'v807.d@example.com', '+6581000204', v_service,
    1, (v_day + time '16:00') at time zone 'Asia/Singapore', null, null, false,
    encode(sha256(convert_to('v807:t5:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v807:i5:' || v_business::text, 'UTF8')), 'hex'),
    encode(sha256(convert_to('v807:f5:' || v_business::text, 'UTF8')), 'hex'),
    null, v_staff, v_branch);
  v_request := nullif(v_res->>'request_id','')::uuid;
  perform pg_temp.as_v807_user(v_owner);
  v_err := null;
  ...
  perform pg_temp.as_v807_user(v_outsider);
  v_err := null;
  begin
    perform public.staff_reschedule_and_confirm_booking_request_v329(
