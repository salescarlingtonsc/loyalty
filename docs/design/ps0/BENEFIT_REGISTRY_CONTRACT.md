...
| `tier` (PS-3) | `tier_entry:{business_id}:{client_id}:{tier}` (§12 once-ever) | `program_entitlements(id)` / `client_tier_status` (future) | tier-entry reward config (benefit snapshot) | snapshot × `margin_band`; medium |
