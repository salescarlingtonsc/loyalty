# V155 Safe Multi-Branch Foundation

## Result

V155 raises multi-branch readiness from **58 / 100** to **74 / 100** for the implemented foundation.

Post-V155 readiness score: 74 / 100.
...
The reporting scope controls analytics and audience preparation. V155 supports:
...
One customer is a business-level customer. The same canonical customer visiting multiple branches remains one customer. V155 deduplicates by canonical customer ID, not phone text in frontend code. V155 does not merge duplicate historical customer records.
...
V155 adds the canonical server-side promotion branch guard
`assert_promotion_redeemable_at_branch_v155`. Any generic promotion redemption
workflow must call this guard before completing a redemption. The current
customer promotion module is display/discovery content; the existing redeemable
QR offer flow remains `redeem_growth_offer_v108`, which already rejects a sale
whose branch differs from the issued offer branch.

The V155 promotion guard verifies:
...
Campaign delivery remains disabled. V155 campaign preparation supports audience counts for:
...
Branch expenses are included when branch-attributed rows are in the selected reporting scope. Business-wide overhead is not silently allocated across selected branches. V155 does not change cash-basis, accrual, expense, or P&L formulas.

## Loyalty, credit, and package scope

Loyalty points and credits remain business/customer scoped. V155 does not make points or credit branch-specific.

Package portability remains ambiguous. V155 does not change package purchase, redemption, or session-portability semantics and does not claim packages work across all branches unless existing enforcement proves that case.
