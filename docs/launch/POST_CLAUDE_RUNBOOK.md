...
3. **Apply the complete immutable migration chain in canonical order.** The reviewed chain currently spans v1 through v49a and contains 77 canonical migration files. Treat `supabase/canonical-migration-order.manifest.json` and its companion SHA-256 digest as the deploy-order authority, with `db/migrations/migration-order.manifest.json` preserving source-file coverage and hashes. Run `npm run migration-manifest:check` and `npm run canonical-migrations:check`, apply and test the exact materialized `supabase/migrations` chain on rehearsal first, then apply that same approved chain to the final target. Never hand-select a subset or infer order from filenames. `supabase/functions` is only for Edge Function deployment; it is not a migration source. Re-run schema and data parity after each logically coupled migration group. A manifest match or successful SQL execution is not closure by itself; retain the post-apply catalog, grant, RLS, and behavior evidence.
