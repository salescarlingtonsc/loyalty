...
| AO-2 | **API-driven WhatsApp** — the Inbox (v768), the "WhatsApp automation" card, the Bring-back "WhatsApp delivery" strip, the Customer 360 "WhatsApp offers" consent row, the customer wallet "WhatsApp from <business>" opt-in, and any analytics that name WhatsApp | "I want to hide everything WhatsApp-inbox related, except for manual WhatsApp click. Those API WhatsApp I want it hide." | 2026-09-08 (Inbox + analytics first ruled 2026-09-05) | Hidden in the business app and the customer app behind one switch, `HIDE_WHATSAPP_API_SURFACES_V824` (v768's `RETIRED_BUSINESS_MODULES_V768` still covers the Inbox). **Kept on purpose:** every *manual* WhatsApp click — the booking-confirmation button, the share sheet's WhatsApp channel, Bring-back's "copy the list and message them yourself" — because those open WhatsApp on the person's own phone and Peekaa sends nothing. Server-side: the platform master switch `whatsapp_outbound` (and `whatsapp_retention_sends`) is **off** as of nestly_v824 (2026-09-08, owner: "this whatsapp is good, i just do not want the API whatsapp for now"), so the appointment triggers and dispatchers refuse with `outbound_not_enabled`; templates, dispatch functions and triggers are left in place, nothing dropped, and un-doing is one flag. |
| AO-3 | **Staff commission rules** (FL-PAYROLL) | "Verify the end to end sales commission works perfectly with data accuracy … ensure staff commission is accurate. While staff commission %, how accurate is it? Because it is now based on staff % multiply by the total revenue sold." | 2026-09-09 | Rules pinned by `db/tests/v850_commission_accuracy_end_to_end.sql` (nestly_v850, applied as v832; objects keep _v832): commission is **per line**, frozen when the sale is recorded; a service/product/package line pays its own rate else the member's service/product %, a fixed amount beats a %, a bundle rate overrides its members and a bundle fixed amount is shared once per bundle; a **discount line reduces commission at the bill's own blended % rate** (fixed-amount lines are not reduced), so a member nets exactly their % of what the customer paid; a **package pays the member the till picked** (sell_package_v832), once, sessions pay nothing; a **gift card pays nothing**; commission starts on `commission_starts_on`; a reversed sale is shown and counted nowhere. **Open, needs a ruling:** a custom "Other item" line pays the member's product % (v827 left it so) — say if custom lines should pay 0 or a rate of their own. |
