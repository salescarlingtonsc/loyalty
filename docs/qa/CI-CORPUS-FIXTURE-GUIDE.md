...
| `public.payments` | **a two-GUC pair**: `app.payment_insert_id` (= the new row's own id) AND `app.payment_write_scope`, one of `record_payment` / `credit_tender` / `sale_reversal`. The scope constrains the row: `record_payment` takes `payment`/`deposit`/`no_show_fee` with any method but `credit`/`gift_card`; `sale_reversal` takes ONLY a negative `refund` in `cash` or `credit` with a `sale_id`. Leave `branch_id` NULL unless the fixture wants a cash-drawer session — `app.on_payment_drawer` opens one for every cash payment that has a branch. (found on v772) |
| `public.points_ledger` | **a two-GUC pair**: `app.points_ledger_insert_id` + `app.points_ledger_write_scope`, and the scope dictates the row's shape (`redemption_reversal` = `entry_type 'adjust'`, positive points, `sale_id` null, `actor` = `auth.uid()`, so a JWT must be set first). `redemption_reversal` additionally needs `app.redemption_reversal_config_version_id`. `credit_ledger` follows the same pattern with its own two GUCs. (found on v772) |
| `public.loyalty_operations` | `app.loyalty_operation_insert_id` (= the new row's own id) on INSERT; `app.loyalty_operation_complete_id` for the one legal `reserved -> completed` UPDATE. A hand-built `loyalty_redemption_reversals` row needs this whole chain: an operation, a `points_ledger` row, then `loyalty_redemption_provenance` (whose `operation_id` is a composite FK into the operation and whose `config_version_id` is NOT NULL, so the tenant needs a published `firm_config_versions` row — see v423's recipe). (found on v772) |
