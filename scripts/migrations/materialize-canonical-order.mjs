...
assert.equal(plan.items.length, 491, // + v602 same-business composite FKs (SEC-02 B1+B2) // + v601 pause the nine featureless cron jobs (real ledger stamp 20260829102308) // + v600 register the manually-created v361 bring-back cron job in source // + v590-v592 eight source-recovery mirrors of already-applied production migrations (cron history retention, webhook consumer markers, support-tick dispatcher; drift closure 2026-08-29) // + v599 the browser roles lose the writes nothing in the product uses (SEC-01 reward_grants, notifications, resources, SEC-09 anon/internal execute) // + v598 the shop's opening hours are every teammate's default working hours (owner ruling: Cubbly's Sunday) // + v593 a package can be given a life (owner photo 5) // + v589 referral switch is one switch // + v588 staff signup flow integrity // + v587 join QR names its business // + v586 tier basis reaches the draft // + v581 short-notice/reschedule, v582 attribution wiring, v583 owner toggles
