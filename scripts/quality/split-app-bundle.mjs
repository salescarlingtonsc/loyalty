...
export const BUSINESS_ENTRIES = [
  'renderShell', 'renderOnboard', 'renderAuth', 'renderPersonaChoice',
  'renderPersonaResolutionUnavailable', 'renderBusinessWorkspaceControl',
  'renderWorkspaceAccessUnavailable', 'renderApprovedBusinessActivation',
  'renderBusinessStaffInviteAcceptV151', 'renderStaffInviteAuthV151',
  'renderBusinessSignupChoice', 'loadWorkspaceLocaleV97'
];

export function partition(source) {
  const { blocks, byName } = buildGraph(source);
  const missing = [...CUSTOMER_ENTRIES, ...BUSINESS_ENTRIES].filter((name) => !byName.has(name));
  if (missing.length) throw new Error(`surface entry points not found in ${SOURCE}: ${missing.join(', ')}`);

  const severed = new Map([['route', new Set([...CUSTOMER_ENTRIES, ...BUSINESS_ENTRIES])]]);
  ...
  for (const block of blocks) {
    ...
    if (inCustomer && inBusiness) { placement.set(block, 'core'); continue; }
  ...
  const orphans = blocks.filter((block) => block.name
    && !core.has(block.name) && !customer.has(block.name) && !business.has(block.name)
    && !I18N_TABLES.includes(block.name));
  const home = new Map();
  ...
  for (let pass = 0; pass < 8; pass += 1) {
    ...
    for (const block of orphans) {
      ...
      for (const ref of block.refs) {
        ...
        if (where === 'customer' || where === 'business') surfaces.add(where);
      }
  ...
  return { blocks, placement, byName, core, customer, business, orphans };
}

/**
 * The invariant the whole split rests on: no chunk may reference a symbol that lives in a
 * different, independently loaded chunk. The two sanctioned exceptions are the router's dispatch
 * to the surface entry points (it awaits the chunk first) and the translation tables (their one
 * reader guards with `typeof`).
 */
export function crossChunkViolations(source) {
  const { blocks, placement } = partition(source);
  const home = new Map();
  for (const block of blocks) if (block.name) home.set(block.name, placement.get(block));
  const dispatched = new Set([...CUSTOMER_ENTRIES, ...BUSINESS_ENTRIES]);
  const violations = [];
  for (const block of blocks) {
...
export function renderChunks(source) {
  const { blocks, placement } = partition(source);
  const parts = { core: [], customer: [], business: [], i18n: [] };
