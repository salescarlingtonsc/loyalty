  ...
  return drainBoundedKeysetPages<LocalSubscription>({
    ...
    fetchPage: async (after, limit) => {
      ...
      let query = admin
        ...
        .like('provider_subscription_id', 'sub_%')
...
/* nestly_v791: the businesses whose company subscription or any branch subscription is on
   Stripe. Rows from other providers share the mirror tables and are outside this run. */
async function stripeTenantScope(admin: ReturnType<typeof billingAdminClient>): Promise<string[]> {
  const ids = new Set<string>();
  const { data: company, error: companyError } = await admin
    .from('subscriptions').select('business_id').eq('billing_provider', 'stripe').limit(20000);
  if (companyError) throw new Error('stripe tenant scope unavailable');
  for (const row of (company || []) as Array<{ business_id: string }>) ids.add(String(row.business_id));
  const { data: branch, error: branchError } = await admin
    .from('branch_subscriptions_v786').select('business_id').eq('provider', 'stripe').limit(20000);
  if (branchError) throw new Error('stripe tenant scope unavailable');
  for (const row of (branch || []) as Array<{ business_id: string }>) ids.add(String(row.business_id));
  return [...ids];
}

/* nestly_v764 on Stripe: "Stop" is a local intent until the due-date sweep, and Stripe's
   cancel_at_period_end is what the sweep sets. Marking follows the provider call, only on
   success; failures are counted, never thrown. */
async function runDueRenewalCancelsStripe(
  admin: ReturnType<typeof billingAdminClient>,
  stripe: Stripe,
): Promise<{ attempted: number; sent: number; failed: number }> {
  const counts = { attempted: 0, sent: 0, failed: 0 };
  const { data, error } = await admin.rpc('list_due_renewal_cancels_v764', {});
  if (error) { counts.failed += 1; return counts; }
  const envelope = data && typeof data === 'object' && !Array.isArray(data) ? data as Record<string, unknown> : null;
  const rows = (envelope && Array.isArray(envelope.due) ? envelope.due : Array.isArray(data) ? data : []) as Array<Record<string, unknown>>;
  for (const row of rows.slice(0, 50)) {
    const subscriptionId = String(row.provider_subscription_id || '');
    if (!subscriptionId.startsWith('sub_') || String(row.billing_provider || 'stripe') !== 'stripe') continue;
    counts.attempted += 1;
    try {
      await stripe.subscriptions.update(subscriptionId, { cancel_at_period_end: true });
      const mark = row.branch_id
        ? await admin.rpc('mark_branch_renewal_cancel_sent_v786', { p_branch: String(row.branch_id) })
        : await admin.rpc('mark_renewal_cancel_sent_v764', { p_business: String(row.business_id) });
      if (mark.error) throw new Error('renewal cancel mark failed');
      counts.sent += 1;
    } catch {
      counts.failed += 1;
    }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    /* nestly_v791: only the tenants on Stripe are in scope — the Razorpay mirror rows from
       September stay in the same tables and must never be looked up at Stripe. */
    const scopeIds = await stripeTenantScope(admin);
    ...
    const { data: runId, error: startError } = await admin.rpc(
      ...
      { p_run_mode: 'scheduled', p_cursor_start: JSON.stringify(cursor), p_provider: 'stripe' },
    );
    if (startError || !runId) {
    ...
    try {
      ...
      const renewalCancels = await runDueRenewalCancelsStripe(admin, stripe);
      ...
      const summary = {
        partial,
        provider: 'stripe',
        scoped_businesses: scopeIds.length,
