...
Deno.serve(async (req) => {
  ...
  try {
    ...
    /* nestly_v791: two endpoint secrets can sign a genuine delivery — the platform's, and the
       TEST-mode endpoint's when demo firms are routed to Stripe test mode. Which one verified says
       which mode the event must claim: a test-endpoint event is never live. */
    const secrets: Array<{ label: 'live' | 'test'; secret: string }> = [
      { label: 'live', secret: requiredEnv('STRIPE_WEBHOOK_SECRET') },
      { label: 'test', secret: (Deno.env.get('STRIPE_TEST_WEBHOOK_SECRET') || '').trim() },
    ].filter((candidate) => candidate.secret);
    let event: Stripe.Event | null = null;
    let matched: 'live' | 'test' = 'live';
    let lastError: unknown = null;
    for (const candidate of secrets) {
      try {
        event = await stripe.webhooks.constructEventAsync(
          rawBody, signature, candidate.secret, undefined, Stripe.createSubtleCryptoProvider(),
        );
    ...
    const wantLivemode = matched === 'test' ? false : expectedLivemode(secretKey);
