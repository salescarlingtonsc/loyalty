-- FRENLY — initial schema (0001_frenly_init, tables → functions → RLS)
...
create index on public.stock_batches (product_id, expires_on);
...
create policy staff_insert on public.staff for insert to authenticated
  with check (app.is_salon_owner(salon_id) or not exists
    (select 1 from public.staff s where s.salon_id = staff.salon_id));
...
create policy credit_ledger_insert on public.credit_ledger for insert to authenticated
  with check (app.is_salon_member(salon_id));
