-- FRENLY v2 — industry-agnostic SaaS: tenancy rename, sales, points engine,
-- retention programs, booking requests, public portal RPCs, automation triggers.

-- 1) Tenancy: salons → businesses (multi-industry)
...
-- helper fns: same names/signatures (policies keep OIDs), bodies updated
...
-- 2) Clients: demographics + consent (dashboard: age group / gender)
...
-- 5) Points ledger (append-only; points ≠ credit)
...
-- 7) Public booking requests (customer portal)
...
-- 9) Automations: earn points + retention rewards on every sale
...
-- 10) Redeem points → mint credit (atomic, checked)
...
-- 11) Public portal RPCs (anon: read business card + submit booking request only)
