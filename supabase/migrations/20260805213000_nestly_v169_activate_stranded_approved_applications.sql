...
/*
  ...
  This migration has two parts:
    ...
    Part 1 - a super-admin RPC that finishes activation for exactly those stranded rows, so a
             stranded owner ends up indistinguishable from one approved after V167.

    Part 2 - repairs `platform_decide_business_application_v105` itself, which turned out to be
             broken for EVERY approval: it seeded public.loyalty_programs, tripping
             app.c45_loyalty_program_version_write_guard (which demands auth.uid() be an owner
             staff row of the new business, while auth.uid() during approval is the super
             admin), so the whole approval transaction rolled back. audit_log contained zero
             DIRECT_APPLICATION_APPROVED_ACTIVATED_V167 rows as of 2026-08-05, confirming no
             approval had ever succeeded. Part 2 drops the seed; full rationale is inline.
*/
...
begin
  ...
  /*
    ...
    The same insert is removed from public.platform_decide_business_application_v105 in Part 2
    of this migration, which is what unblocks every FUTURE approval.
  */
...
/* V169 part 2 - unblock ALL future approvals (rationale inside the function). */
