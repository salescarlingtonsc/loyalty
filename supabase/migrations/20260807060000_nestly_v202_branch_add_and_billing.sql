...
-- create-or-replace preserves existing grants, but the preflight requires every shipped
-- SECURITY DEFINER overload to state its own revoke rather than inherit one. Restating it
-- here means the guarantee travels with the file that ships the function.
revoke all on function public.request_billing_command_v124(uuid,text,text,integer,uuid) from public, anon;
grant execute on function public.request_billing_command_v124(uuid,text,text,integer,uuid) to authenticated;
