...
-- Definer-only, matching every sibling sme_* table: a new table inherits
-- Supabase's default browser-role grants, and RLS-with-no-policies would be the
-- only thing standing between anon and the row set. Take the grants away so a
-- future permissive policy cannot silently open it; the review RPCs are
-- SECURITY DEFINER and reach it regardless.
revoke all on public.sme_merchant_match_candidates from anon, authenticated;
