...
-- v350 re-creates this reader (adding custom_label to its payload), so it restates the ACL on its
-- exact overload: each pending migration must be readable on its own as revoking PostgreSQL's
-- default PUBLIC execute grant, not depend on v215 having done it.
