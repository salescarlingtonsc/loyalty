/* nestly_v692 — two readers that answer with the wrong rows.
...
begin
  ...
  loop
    ...
    if position(v_site.replacement in v_def) > 0 then
      raise notice 'nestly_v692: % already sums every pot in business_pot scope, skipping', v_site.fn;
      continue;
    end if;
    if (length(v_def) - length(replace(v_def, v_site.needle, '')))
       / nullif(length(v_site.needle),0) <> v_site.occurrences then
      raise exception 'nestly_v692: % did not carry its pot predicate exactly % time(s)'
        , v_site.fn, v_site.occurrences using errcode = 'XX001';
    end if;
    v_new := replace(v_def, v_site.needle, v_site.replacement);
    if v_new = v_def then
      raise exception 'nestly_v692: the pot splice on % produced no change', v_site.fn
        using errcode = 'XX001';
    end if;
    execute v_new;
    v_fixed := v_fixed + 1;
  end loop;
  raise notice 'nestly_v692: % pot predicate site(s) corrected', v_fixed;
end
$v692_pots$;
...
begin
  ...
  if position('v_all_rows' in v_def) > 0 then
    raise notice 'nestly_v692: the engagement report already totals the whole range, skipping';
  else
    if (length(v_def) - length(replace(v_def, v_decl, ''))) / nullif(length(v_decl),0) <> 1
      ...
      raise exception 'nestly_v692: an engagement anchor did not match exactly once — body drifted'
        using errcode = 'XX001';
    end if;
    ...
    if v_new = v_def then
      raise exception 'nestly_v692: the engagement splice produced no change' using errcode = 'XX001';
    end if;
    execute v_new;
  end if;
end
...
begin
  ...
  loop
    ...
    if position('= ''programme_pot'' and' in v_def) > 0 then
      raise exception 'nestly_v692 (F079): % still zeroes every balance in business_pot scope'
        , v_site.fn using errcode = 'XX001';
    end if;
    if position('<> ''programme_pot'' or' in v_def) = 0 then
      raise exception 'nestly_v692 (F079): % lost its pot scope rule entirely', v_site.fn
        using errcode = 'XX001';
    end if;
  end loop;
  if position('into v_rows,v_total,v_all_rows' in v_engagement) = 0
    ...
    raise exception 'nestly_v692 (F123): the engagement tiles still read one page'
      using errcode = 'XX001';
  end if;
  if position('jsonb_array_elements(v_rows)' in v_engagement) > 0 then
    raise exception 'nestly_v692 (F123): an aggregate still reads the truncated page'
      using errcode = 'XX001';
  end if;
end
$verify$;
