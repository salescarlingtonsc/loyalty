-- nestly_v693 — deleting a stamp gift stops paying out mid-card (audit F038).
--
...
--
-- ONE AUTHORITY FOR THE PREDICATE. app.reward_live_on_offer_v693(active, withdrawn_at, is_stamp)
...
-- and re-splice each anchor back (drop the app.reward_live_on_offer_v693 call, restore `live.active`
...
comment on column public.loyalty_rewards.withdrawn_at is
  'nestly_v693: set when business_delete_reward_v326 withdrew this stamp gift version-forward. '
  ...
  'gift was deleted before v693 (or is a points gift) and stays deleted for everyone.';
...
create or replace function app.loyalty_reward_withdrawn_guard_v693()
returns trigger
language plpgsql
security definer
set search_path to 'pg_catalog','public','app','pg_temp'
...
revoke all on function app.loyalty_reward_withdrawn_guard_v693() from public, anon, authenticated;

drop trigger if exists trg_loyalty_rewards_withdrawn_v693 on public.loyalty_rewards;
create trigger trg_loyalty_rewards_withdrawn_v693
before insert or update on public.loyalty_rewards
for each row execute function app.loyalty_reward_withdrawn_guard_v693();
...
-- =============================================================================================
create or replace function app.reward_live_on_offer_v693(
...
comment on function app.reward_live_on_offer_v693(boolean, timestamptz, boolean) is
  'nestly_v693: is this loyalty_rewards row still on offer? Active rows always are. A withdrawn '
...
revoke all on function app.reward_live_on_offer_v693(boolean, timestamptz, boolean)
...
begin
  ...
  -- nestly_v693: the same question business_update_reward_v326 asks before it splits.
  v_is_stamp := exists (
...
comment on function public.business_delete_reward_v326(uuid, uuid) is
  'v693: remove a gift. A stamp gift on a running stamp programme is withdrawn VERSION-FORWARD '
...
'      where app.reward_live_on_offer_v693(live.active, live.withdrawn_at, shape.is_stamp)
...
begin
  ...
  if position('reward_live_on_offer_v693' in v_def) > 0 then
    raise notice 'nestly_v693: reward_availability_v432 already asks the v693 predicate, skipping';
  else
    if (length(v_def) - length(replace(v_def, v_arm0_join, ''))) / nullif(length(v_arm0_join),0) <> 1
      ...
      raise exception 'nestly_v693: an anchor did not match exactly once in reward_availability_v432 — body drifted'
        using errcode = 'XX001';
    end if;
    ...
    if v_new = v_def then
      raise exception 'nestly_v693: splice produced no change in reward_availability_v432'
        using errcode = 'XX001';
    end if;
    execute v_new;
  end if;
...
-- 4b. app.redeem_reward_core — the counter. The live-row veto becomes the v693 predicate; the
...
'  if not app.reward_live_on_offer_v693(v_reward.active, v_reward.withdrawn_at,
...
begin
  ...
  if position('reward_live_on_offer_v693' in v_def) > 0 then
    raise notice 'nestly_v693: redeem_reward_core already asks the v693 predicate, skipping';
  else
    if (length(v_def) - length(replace(v_def, v_anchor, ''))) / nullif(length(v_anchor),0) <> 1 then
      raise exception 'nestly_v693: anchor did not match exactly once in redeem_reward_core — body drifted'
        using errcode = 'XX001';
    end if;
...
begin
  ...
  if position('reward_live_on_offer_v693' in v_def) > 0 then
    raise notice 'nestly_v693: customer_get_stamp_card_v323 already asks the v693 predicate, skipping';
  else
    if (length(v_def) - length(replace(v_def, v_anchor, ''))) / nullif(length(v_anchor),0) <> 1 then
      raise exception 'nestly_v693: anchor did not match exactly once in customer_get_stamp_card_v323 — body drifted'
        using errcode = 'XX001';
    end if;
    v_new := replace(v_def, v_anchor, v_inject);
    execute v_new;
...
'    select * into v_reward from public.loyalty_rewards reward
    where reward.id=p_reward and reward.business_id=p_business and not reward.paused
      and app.reward_live_on_offer_v693(reward.active, reward.withdrawn_at,
            exists (select 1 from public.business_programmes spine
...
begin
  ...
  if position('reward_live_on_offer_v693' in v_def) > 0 then
    raise notice 'nestly_v693: customer_create_redemption_intent_v89 already asks the v693 predicate, skipping';
  else
    if (length(v_def) - length(replace(v_def, v_anchor, ''))) / nullif(length(v_anchor),0) <> 1 then
      raise exception 'nestly_v693: anchor did not match exactly once in customer_create_redemption_intent_v89 — body drifted'
        using errcode = 'XX001';
    end if;
    v_new := replace(v_def, v_anchor, v_inject);
    execute v_new;
...
begin
  ...
  if position('reward_live_on_offer_v693' in v_def) > 0 then
    raise notice 'nestly_v693: stamp_reward_expire_due_v464 already asks the v693 predicate, skipping';
  else
    if (length(v_def) - length(replace(v_def, v_anchor, ''))) / nullif(length(v_anchor),0) <> 1 then
      raise exception 'nestly_v693: anchor did not match exactly once in stamp_reward_expire_due_v464 — body drifted'
        using errcode = 'XX001';
    end if;
    v_new := replace(v_def, v_anchor, v_inject);
    execute v_new;
...
begin
  if not exists (select 1 from information_schema.columns
    ...
    raise exception 'nestly_v693: loyalty_rewards.withdrawn_at was not created' using errcode='XX001';
  end if;
  if not exists (select 1 from information_schema.column_privileges
    ...
    raise exception 'nestly_v693: authenticated cannot SELECT withdrawn_at — select(*) reads of loyalty_rewards would fail'
      using errcode='XX001';
  end if;
  if not exists (select 1 from pg_trigger t
                  where t.tgrelid='public.loyalty_rewards'::regclass
                    and t.tgname='trg_loyalty_rewards_withdrawn_v693'
                    and not t.tgisinternal) then
    raise exception 'nestly_v693: withdrawn_at has no write guard — it must have exactly one writer'
      using errcode='XX001';
  end if;
  foreach v_name in array array[
    ...
    if position('reward_live_on_offer_v693' in pg_get_functiondef(v_name::regprocedure)) = 0 then
      v_missing := v_missing || v_name;
    end if;
  end loop;
  if array_length(v_missing,1) > 0 then
    raise exception 'nestly_v693: these readers still gate on the live row alone: %',
  ...
  if position('stamp_config_edit_begin_v433' in pg_get_functiondef(
    ...
    raise exception 'nestly_v693: deleting a stamp gift still does not version forward'
      using errcode='XX001';
  end if;
  if exists (select 1 from information_schema.routine_privileges
              where routine_schema='app' and routine_name='reward_live_on_offer_v693'
                and grantee in ('anon','authenticated','PUBLIC')) then
    raise exception 'nestly_v693: the predicate is reachable from the API' using errcode='XX001';
  end if;
end
$verify$;
