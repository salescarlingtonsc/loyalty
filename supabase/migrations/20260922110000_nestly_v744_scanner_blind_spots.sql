...
-- nestly_v685 (Singapore-day authority) patched public.refresh_growth_recommendation_v108. Restating that capture would
...
begin
  ...
  if (length(v_before) - length(replace(v_before, v_old1, ''))) / length(v_old1) <> 1 then
    raise exception 'public.refresh_growth_recommendation_v108: exclusion anchor must occur exactly once in the live body';
  end if;
...
-- ACL restated verbatim for the re-emitted spine (nestly_v718 form; CREATE OR REPLACE keeps the
-- live ACL, the preflight guard requires every pending migration to say so itself).
...
-- The other re-emitted readers restate the ACL their latest emitting migration set (v743, v740,
-- v734, v70, v742); nothing widens or narrows here.
