/* nestly_v808 — the marketing choice can be saved by a customer who has no preferences row yet,
...
begin
  ...
  /* nestly_v808 (audit F040): this was a bare UPDATE, and a customer whose identity was created
...
begin
  if position('insert into public.customer_registration_preferences' in v_def) = 0
    ...
    raise exception 'nestly_v808: the marketing-choice write is not an upsert'
      using errcode = 'XX001';
  end if;
  /* The row must still be the caller's own. */
  if position('where pref.auth_user_id = v_actor' in v_def) = 0 then
    raise exception 'nestly_v808: the upsert no longer restricts itself to the caller''s own row'
      using errcode = 'XX001';
  end if;
  /* A refusal must still be a refusal. */
  if position('if not found then' in v_def) = 0
     or position('errcode = ''42501''' in v_def) = 0 then
    raise exception 'nestly_v808: a write that matched nothing no longer raises'
      using errcode = 'XX001';
  end if;
  if position('''app.v92_marketing_source'', ''customer_profile''' in v_def) = 0 then
    raise exception 'nestly_v808: the consent evidence source is no longer declared'
      using errcode = 'XX001';
  end if;
  ...
  if not exists (
    ...
    raise exception 'nestly_v808: the scope-stamp trigger no longer covers INSERT'
      using errcode = 'XX001';
  end if;
  if not exists (
    select 1 from pg_trigger t
    ...
    raise exception 'nestly_v808: the consent-evidence trigger no longer covers INSERT'
      using errcode = 'XX001';
  end if;
  if exists (select 1 from information_schema.routine_privileges
    ...
    raise exception 'nestly_v808: the marketing-choice RPC became anonymously reachable'
      using errcode = 'XX001';
  end if;
end
$verify$;
