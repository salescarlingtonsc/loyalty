...
export function buildV130SelfServeVisual(app){
  ...
  const sourceHash=createHash('sha256').update([style,signup,onboard,control].join('\n')).digest('hex');
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="production-source-sha256" content="${sourceHash}"><link rel="icon" href="data:,"><title>Nestly V130 actual-renderer browser acceptance</title><style>${style}body{padding:16px}.v130-nav{position:fixed;z-index:300;top:8px;right:8px;display:flex;gap:6px}.v130-nav .btn{padding:8px 12px}.center-wrap{padding-top:68px}@media(max-width:600px){body{padding:0}.v130-nav{position:relative;top:auto;right:auto;padding:8px;overflow:auto}.center-wrap{padding:12px}.grid2{grid-template-columns:1fr}}</style></head><body><nav class="v130-nav" aria-label="V130 acceptance views"><button class="btn" data-view="signup">Account</button><button class="btn ghost" data-view="setup">Setup</button><button class="btn ghost" data-view="pending">Pending</button><button class="btn ghost" data-view="control">Control</button></nav><div id="root"></div><script>
  ...
  const brandWordmark=()=>'<span>Nestly</span>';const legalLinks=()=>'<div class="legal-links"><a href="#">Terms</a> · <a href="#">Privacy</a></div>';
  ...
  const NestlyNativeBridge={publicUrl:path=>'https://nestly.asia'+path};const WORKSPACE_LOCALES_V97=['en','zh-CN','ms'];
  const renderAuth=()=>{},route=()=>{},resetClientSessionState=()=>{},killChannels=()=>{},nav=value=>{window.__lastNavigation=value},toast=()=>{};
  const CUI={icon:()=>''};const S={hasCustomerPersona:false};
  let currentView='signup';window.__consoleErrors=[];window.__rpcCalls=[];
  ...
  const modules=['Customer CRM and QR signup','Loyalty points, stamps and rewards','Appointments, team schedule and waitlist','Promotions and AI-assisted promotion copy'];
