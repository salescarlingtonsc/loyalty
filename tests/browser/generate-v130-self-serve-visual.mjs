...
export function buildV130SelfServeVisual(app){
  ...
  const sourceHash=createHash('sha256').update([style,oauth,signup,onboard,control,deletion].join('\n')).digest('hex');
  ...
  const NestlyNativeBridge={publicUrl:path=>'https://www.peekaa.asia'+path};const WORKSPACE_LOCALES_V97=['en','zh-CN','ms'];
