...
export function buildV131StoreVisual(app){
  ...
  const sb={auth:{signOut:async()=>{window.__signOuts+=1;return {}}},rpc:async(name,args)=>{window.__rpcCalls.push({name,args});if(name==='get_account_deletion_request_v131'){if(fixtureState==='load-error')return {data:null,error:{message:'synthetic load error'}};if(fixtureState==='pending')return {data:pendingRequest,error:null};if(fixtureState==='completed-deleted')return {data:completedDeletedRequest,error:null};if(fixtureState==='completed-retained')return {data:completedRetainedRequest,error:null};return {data:null,error:null}}if(name==='request_account_deletion_v131'){if(fixtureState==='submit-error')return {data:null,error:{message:'synthetic submit error'}};return {data:pendingRequest,error:null}}return {data:null,error:null}}};
