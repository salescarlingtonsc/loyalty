...
const source = await readFile(root + 'app/platform-console.js', 'utf8');
const ctx = { Object, Map, Set };
ctx.globalThis = ctx;
vm.runInNewContext(source, ctx, { filename: 'platform-console.js' });
...
await writeFile(root + 'tests/browser/v181-onboarding-board.html', `<!doctype html><meta charset="utf-8">
...
console.log('kanban cards:', (kanban.match(/platform-prospect-card-compact/g) || []).length,
  '| draggable:', (kanban.match(/draggable="true"/g) || []).length,
  '| list rows:', (list.match(/platform-prospect-row/g) || []).length);
