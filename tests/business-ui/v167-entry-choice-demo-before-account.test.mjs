...
test('V167 public business signup asks for intent before collecting business data', () => {
  ...
  assert.match(choice, /Stripe Checkout or manual payment approval/);
...
test('V167 set-up-business copy separates Stripe activation from manual approval', () => {
  assert.match(ownerSignup, /Create your Peekaa owner account/);
  assert.match(ownerSignup, /choose Stripe Checkout or manual payment approval/);
  assert.match(ownerSignup, /Stripe opens access only after verified payment/);
  assert.match(ownerSignup, /manual payment waits for Super Admin approve\/reject/);
  assert.match(ownerSignup, /Stripe payment auto-activates after verified payment/);
...
test('V167 manual payment remains authenticated Super Admin approve or reject flow', () => {
  ...
  assert.match(manualFallback, /bank transfer, cash, or another manual arrangement instead of Stripe/);
  assert.match(manualFallback, /Super Admin reviews and approves or rejects this manual-payment application/);
  assert.match(manualFallback, /does not activate a workspace, open Stripe Checkout, or record payment/);
