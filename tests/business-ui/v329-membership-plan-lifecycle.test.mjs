...
test('V329 read-only staff (canWrite=false) gets no Delete button, and delete/tab wiring bails out before touching them', () => {
  ...
  assert.match(rowFn, /\$\{canWrite\?`<button class="btn ghost sm" onclick="togglePlan\('\$\{p\.id\}',\$\{!p\.active\}\)">\$\{p\.active\?'Turn off':'Turn on'\}<\/button>\s*<button type="button" class="btn danger sm" data-plan-delete-v329="\$\{esc\(p\.id\)\}">Delete<\/button>`:''\}/);
