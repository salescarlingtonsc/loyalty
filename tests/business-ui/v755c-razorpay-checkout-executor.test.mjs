/* nestly_v755 (Stripe swap, builder C / client scope) — replaces the client-facing assertions
   ...
   supabase/functions/stripe-billing-webhook and razorpay-billing-command source, and app.js's
   self-service checkout executor. The webhook/command edge functions are renamed and rewritten
   for Stripe by the migration/edge-function builders in this wave (see
   ...
   Stripe-branded rather than a leftover "Stripe" string. Same coverage intent as the retired
...
test('v755 the checkout executor invokes stripe-billing-command, not the retired razorpay-billing-command', () => {
  assert.doesNotMatch(app, /invoke\('razorpay-billing-command'/,
    'no client code may still call the retired Stripe edge function');
  assert.match(checkoutExecutor, /sb\.functions\.invoke\('stripe-billing-command'/);
});
...
test('v755 every non-redirect outcome re-enables the button and names what to do next, in Stripe copy', () => {
...
test('v755 the poll that waits for provider confirmation is unchanged and still terminates, in Stripe copy', () => {
  /* nestly_v782: the bound moved 15 -> 90 attempts in a9d5694d (nestly_v766) — deliberately, and
     for a measured reason: "the onboarding poll waits up to 3 minutes for the webhook (Stripe
...
test('v755 no client entry point still calls the retired stripe-connect-command (PayNow QR / Connect onboarding)', () => {
  assert.doesNotMatch(app, /stripe-connect-command/,
    'Stripe Connect / PayNow POS has no Stripe SG equivalent and was removed, not ported');
...
test('v755 the billing settings screen offers Cancel/Resume instead of the retired Stripe billing portal', () => {
  assert.doesNotMatch(app, /execute\('create_portal'/,
    'Stripe has no customer billing portal; create_portal must not be requested by the client');
  ...
  /* nestly_v764 (owner ruling 4): a Stripe cancel_at_cycle_end cannot be undone, so cancelling
