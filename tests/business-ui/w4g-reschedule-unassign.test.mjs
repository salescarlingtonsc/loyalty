/* W4G / nestly_v695 — "Anyone available" on a staff reschedule must actually un-assign.
...
test('W4G/v695 picking "Anyone available" asks the server to clear the assignment', () => {
  assert.deepEqual(rescheduleStaffChoiceV695({ value: '' }),
    { p_staff: null, p_clear_staff: true });
});

test('W4G/v695 picking a team member names them and does not clear', () => {
  assert.deepEqual(rescheduleStaffChoiceV695({ value: 's-2' }),
    { p_staff: 's-2', p_clear_staff: false });
});

test('W4G/v695 a form with no staff select never asks to un-assign', () => {
...
test('W4G/v695 the two arguments are never both set — the server refuses that pair', () => {
  for (const select of [{ value: '' }, { value: 's-1' }, null]) {
    ...
    assert.ok(!(sent.p_clear_staff && sent.p_staff !== null),
      'p_clear_staff with a named member is refused by nestly_v695 with 22023');
  }
});

test('W4G/v695 both reschedule forms send the flag, and the old shape is gone', () => {
