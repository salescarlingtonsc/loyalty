...
test('G2: 42501 on a wallet deep link renders the not-joined state, never Retry', () => {
  ...
  /* v319: `silent?undefined:` — a background poll keeps the page the customer is reading. A denial
     on a poll is not new information about a wallet that is already on screen, and the not-joined
     card is still the only answer a real render gives. */
...
test('G3: the shell cache is versioned forward and the app document joins it', () => {
  assert.match(sw, /const CACHE_VERSION='v11-20260813-v298-caption-once'/);
