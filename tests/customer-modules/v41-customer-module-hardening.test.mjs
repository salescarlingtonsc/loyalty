...
test('the SPA has no raw customer-module writes and carries stable retry keys', async () => {
  ...
  for (const rpc of [
    ...
    /* V322 (R1/R4): save_referral_program -> save_referral_program_v322. The pre-v322 money
       signature stays INSTALLED for the CDN window but the shipped bundle no longer calls it, and
       this list is about what the bundle calls. */
    /* nestly_v429 (B3): and on to save_referral_program_v421 — v322 has no argument that can name the reward TYPE v425 made explicit. */
    'staff_list_gift_cards_v145', 'save_referral_program_v421', 'save_membership_plan',
