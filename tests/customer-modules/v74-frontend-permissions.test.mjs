...
test('Team permissions initialize from module_perms and render Off Read Edit rather than module checkboxes',()=>{
  ...
  assert.match(settings,/Unavailable for \$\{esc\(ROLE_LABELS\[s\.role\]\|\|s\.role\)\}: Expenses, P&amp;L, Staff commission and Business Intelligence require a finance-capable role/);
