...
test('C45 wallet UI has a truthful separate consent seam, no DOB disclosure, accessible counter actions, and stale guards', async () => {
  ...
  assert.match(clientDetail, /const birthdayBenefitsEnabled=canReadLoyalty&&customerFeatures\.customer_birthday_benefits===true/i);
  ...
  assert.match(clientDetail, /select\('id,business_id,full_name,phone,email,referral_code,marketing_consent,created_at'\)/i,
