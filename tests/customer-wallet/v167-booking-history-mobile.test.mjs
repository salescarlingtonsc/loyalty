...
test('customer surfaces expose Book again only through booking-enabled paths',()=>{
  ...
  assert.match(app,/group\.bookingEnabled&&group\.business_slug&&tab!=='bookings'/);
  ...
  assert.match(app,/\$\{bookingEnabled\?`<a class="btn sm customer-programme-book customer-programme-contact-item-v337/);
