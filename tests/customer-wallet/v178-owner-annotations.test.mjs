...
test('Home drops the crossed-out page-head title block and keeps Scan to join in the My Rewards heading',()=>{
  ...
  /* v319: the markup moved into a named const so a silent refresh can compare it before painting;
     the string itself, which is what this suite reads, is unchanged. */
  const legacyHome=section('const fallbackHomeMarkupV319=`${customerHomeOffersMarkupV167(offersState)}','wireCustomerHomeOffersV167(()=>renderCustomerWallet())');
