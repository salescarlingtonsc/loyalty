...
test('the gateway validates the availability query and passes the person through',()=>{
  ...
  /* nestly_v882: the same guard also refuses a malformed bundle id, or a bundle alongside a service. */
  assert.match(gateway,/if \(\(service && !UUID_PATTERN\.test\(service\)\) \|\| \(staff && !UUID_PATTERN\.test\(staff\)\)\s*\n\s*\|\| \(branch && !UUID_PATTERN\.test\(branch\)\)\s*\n\s*\|\| \(bundle && \(!UUID_PATTERN\.test\(bundle\) \|\| service\)\)\) return publicError\(req\)/);
