...
test('account creation uses phone and password, then one verification OTP',()=>{
  ...
  assert.match(start,/id="customerSignupConsent" type="checkbox"/);
  assert.doesNotMatch(start,/id="customerSignupMarketing" type="checkbox"/);
  assert.match(start,/if\(!\$\('customerSignupConsent'\)\.checked\)/);
  assert.match(start,/marketingOptedIn:recovering\?false:\$\('customerSignupConsent'\)\.checked/);
  assert.match(start,/agree to receive marketing from \$\{esc\(BRAND\.productName\)\} and selected partners/);
...
test('fresh signup clears abandoned consent while immediate OTP back preserves it',()=>{
  ...
  assert.match(start,/id="customerSignupConsent" type="checkbox" \$\{customerRegistrationState\.legalAccepted&&customerRegistrationState\.marketingOptedIn\?'checked':''\}/);
  assert.doesNotMatch(start,/id="customerSignupMarketing" type="checkbox"/);
  assert.match(start,/rememberCustomerSignupConsent\(!recovering&&\$\('customerSignupConsent'\)\.checked\)/);
  ...
  assert.match(profile,/Your Terms, Privacy Notice, and marketing consent were captured before phone verification/);
  assert.doesNotMatch(profile,/id="customerMarketing" type="checkbox"/);
  assert.match(profile,/if\(!customerSignupConsentRecorded\(\)\)/);
  assert.match(profile,/p_platform_marketing_opted_in:true/);
