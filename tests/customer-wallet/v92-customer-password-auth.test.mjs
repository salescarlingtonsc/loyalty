...
test('normal customer login uses phone and password without touching OTP transport',()=>{
  ...
  /* V388: no captchaToken — Supabase Auth CAPTCHA is off, and normal customer login must not
...
test('fresh signup clears abandoned consent while immediate OTP back preserves it',()=>{
  ...
  assert.match(start,/id="customerSignupConsent" type="checkbox" \$\{customerRegistrationState\.legalAccepted\?'checked':''\}/);
  assert.match(start,/id="customerSignupMarketing" type="checkbox"/);
  ...
  assert.match(profile,/Your Terms and Privacy Notice acceptance was captured before phone verification/);
  assert.doesNotMatch(profile,/id="customerMarketing" type="checkbox"/);
  assert.match(profile,/if\(!customerSignupConsentRecorded\(\)\)/);
  assert.match(profile,/p_platform_marketing_opted_in:customerSignupMarketingOptedIn\(\)/);
