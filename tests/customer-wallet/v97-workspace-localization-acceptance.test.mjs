...
test('v97 generated catalog contains no prompt leakage or executable source fragments',()=>{
  ...
  for(const locale of ['zh-CN','ms']){
    ...
    assert.equal(Object.keys(generatedCopy[locale]).length,5908,`${locale} valid visible-literal inventory changed without catalog review`);
  }
});
...
test('v97 named templates are an exact reviewed inventory with locale and placeholder parity',()=>{
  ...
  assert.equal(keys.length,855,'mixed-interface interpolation inventory changed without review');
...
test('v293 customer portal follows the stored profile language without a separate locale RPC',()=>{
  ...
  /* nestly_v980: the picker is gone and `language` is the constant 'en', so a profile save converges
     a previously-chosen locale back to English rather than preserving it. */
  assert.doesNotMatch(app,/id="customerProfileLanguage"/);
  assert.match(app,/p_preferred_language:language/);
  assert.match(app,/const fullName=\$\('customerProfileName'\)\.value\.trim\(\),language='en';/);
});
