...
test('checked-in canonical plan preserves 45 trusted catalog versions then 184 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 229-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 184);
  assert.equal(manifest.itemCount, 229);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 229);
  ...
  assert.equal(
    ...
    'nestly_v198_join_qr_print_lock'
  );
