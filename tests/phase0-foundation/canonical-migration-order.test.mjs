...
test('checked-in canonical plan preserves 45 trusted catalog versions then 232 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 442-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(
    ...
    'nestly_v572_retention_safety_hardening' // tail — retention lane inert repair
  );
