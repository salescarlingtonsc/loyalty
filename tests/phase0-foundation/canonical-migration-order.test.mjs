...
test('materializer creates one byte-preserving 293-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 248); // V300 growth readbacks
  assert.equal(manifest.itemCount, 293); // V300 growth readbacks
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 293); // V300 growth readbacks
  ...
  assert.equal(
    ...
    'nestly_v300_growth_readbacks' // tail
  );
