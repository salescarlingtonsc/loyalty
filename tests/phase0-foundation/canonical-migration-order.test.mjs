...
test('checked-in canonical plan preserves 45 trusted catalog versions then 213 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 258-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 213);
  assert.equal(manifest.itemCount, 258);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 258);
  ...
  assert.equal(
    ...
    'nestly_v246_personas_direct_resolver'
  );
