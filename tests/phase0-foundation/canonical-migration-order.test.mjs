...
test('checked-in canonical plan preserves 45 trusted catalog versions then 201 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 246-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 201);
  assert.equal(manifest.itemCount, 246);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 246);
  ...
  assert.equal(
    ...
    'nestly_v231_capabilities_follow_the_points_mode'
  );
