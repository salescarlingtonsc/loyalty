...
test('materializer creates one byte-preserving 260-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 215);
  assert.equal(manifest.itemCount, 260);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 260);
  ...
  assert.equal(
    ...
    'nestly_v248_customer_directory_reports_loyalty_availability'
  );
