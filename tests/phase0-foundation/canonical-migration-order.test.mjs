...
test('checked-in canonical plan preserves 45 trusted catalog versions then 101 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 146-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 101);
  assert.equal(manifest.itemCount, 146);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 146);
  ...
  assert.equal(
    ...
    'nestly_v117_effective_loyalty_boundaries'
  );
