...
test('checked-in canonical plan preserves 45 trusted catalog versions then 128 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 173-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 128);
  assert.equal(manifest.itemCount, 173);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 173);
  ...
  assert.equal(
    ...
    'nestly_v134_peekaa_brand_legal'
  );
