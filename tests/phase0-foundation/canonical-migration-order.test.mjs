...
test('checked-in canonical plan preserves 45 trusted catalog versions then 63 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 108-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 63);
  assert.equal(manifest.itemCount, 108);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 108);
  assert.equal(manifest.items[44].version, '20260719190540');
  assert.equal(manifest.items[45].version, '20260721000001');
  assert.equal(manifest.items.at(-1).name, 'frenly_v68c_customer_gift_cards');
