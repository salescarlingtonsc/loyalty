...
test('checked-in canonical plan preserves 45 trusted catalog versions then 179 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 224-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 179);
  assert.equal(manifest.itemCount, 224);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 224);
  ...
  assert.equal(
    ...
    'nestly_v197_persistent_join_qr'
  );
