...
test('materializer creates one byte-preserving 276-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 231); // V278
  assert.equal(manifest.itemCount, 276); // V278
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 276); // V278
  ...
  assert.equal(
    ...
    'nestly_v278_bottle_keep_parity' // V278
  );
