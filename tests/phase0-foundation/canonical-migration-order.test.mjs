...
test('checked-in canonical plan preserves 45 trusted catalog versions then 469 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 521-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(
    ...
    'nestly_v654_receipt_identity_and_perk_date' // tail — v654 UEN normaliser EXECUTE grant + tier-benefit last_used_at
