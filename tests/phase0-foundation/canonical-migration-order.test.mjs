...
test('checked-in canonical plan preserves 45 trusted catalog versions then 154 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 199-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 154);
  assert.equal(manifest.itemCount, 199);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 199);
  ...
  assert.equal(
    ...
    'nestly_v175_marketing_consent_scope_v2'
  );
