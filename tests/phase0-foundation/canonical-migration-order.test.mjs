...
test('checked-in canonical plan preserves 45 trusted catalog versions then 96 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 141-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 96);
  assert.equal(manifest.itemCount, 141);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 141);
  ...
  assert.equal(
    ...
    'nestly_v103_customer_summary_business_identity'
  );
