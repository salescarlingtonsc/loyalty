...
test('checked-in canonical plan preserves 45 trusted catalog versions then 85 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 131-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 86);
  assert.equal(manifest.itemCount, 131);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 131);
  ...
  assert.equal(
    ...
    'nestly_v91_customer_game_notifications'
  );
