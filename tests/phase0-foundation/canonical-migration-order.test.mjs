...
test('materializer creates one byte-preserving 288-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 243); // +v288 +v289
  assert.equal(manifest.itemCount, 288); // +v288 +v289
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 288); // +v288 +v289
  ...
  assert.equal(
    ...
    'nestly_v289_reschedule_respects_business_setting' // tail
  );
