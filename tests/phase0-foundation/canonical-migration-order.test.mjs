...
test('checked-in canonical plan preserves 45 trusted catalog versions then 214 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 259-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 214);
  assert.equal(manifest.itemCount, 259);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 259);
  ...
  assert.equal(
    ...
    'nestly_v247_explore_nearest_first'
  );
