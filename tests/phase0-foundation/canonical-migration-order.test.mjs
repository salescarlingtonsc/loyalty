...
test('checked-in canonical plan preserves 45 trusted catalog versions then 470 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 528-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(
    ...
    'nestly_v664_capacity_tiers_and_manual_payment' // tail — v664 capacity tiers charged per branch and manual-payment billing dates; v663 customer appointment changes before it — v663 customer appointment changes; v661 was applied earlier at its real ledger timestamp
