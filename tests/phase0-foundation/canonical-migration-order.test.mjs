...
test('materializer creates one byte-preserving 291-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 246); // +v288 +v289
  assert.equal(manifest.itemCount, 291); // +v288 +v289
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 291); // +v288 +v289
  ...
  assert.equal(
    ...
    'nestly_v292_demo_requests' // tail
  );
