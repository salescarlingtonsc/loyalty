...
test('materializer creates one byte-preserving 298-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 253); // + v307 programme read model
  assert.equal(manifest.itemCount, 298); // + v307 programme read model
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 298); // + v307 programme read model
