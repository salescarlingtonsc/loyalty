...
test('checked-in canonical plan preserves 45 trusted catalog versions then 127 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 172-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 127);
  assert.equal(manifest.itemCount, 172);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 172);
  ...
  assert.equal(
    ...
    'nestly_v133_operational_closure'
  );
