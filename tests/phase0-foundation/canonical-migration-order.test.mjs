...
test('checked-in canonical plan preserves 45 trusted catalog versions then 122 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 167-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 122);
  assert.equal(manifest.itemCount, 167);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 167);
  ...
  assert.equal(
    ...
    'nestly_v128_simple_rewards_recommender'
  );
