...
test('checked-in canonical plan preserves 45 trusted catalog versions then 173 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 218-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 173);
  assert.equal(manifest.itemCount, 218);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 218);
  ...
  assert.equal(
    ...
    'nestly_v193_manage_unsold_package_plan'
  );
