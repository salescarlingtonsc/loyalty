...
test('materializer creates one byte-preserving 271-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 226); // V271
  assert.equal(manifest.itemCount, 271); // V271
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 271); // V271
  ...
  assert.equal(
    ...
    'nestly_v271_programme_overview' // V271
  );
