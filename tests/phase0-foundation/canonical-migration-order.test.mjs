...
test('checked-in canonical plan preserves 45 trusted catalog versions then 170 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 215-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 170);
  assert.equal(manifest.itemCount, 215);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 215);
  ...
  assert.equal(
    ...
    'nestly_v185_prospect_archive_merge_erase'
  );
