...
test('materializer creates one byte-preserving 320-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 323); // + v311 money kernel + v312 pot migration + v315 lead score repair + v316 taxonomy/match queue repair + v317 restored dependencies + v318 system-managed flag alignment + v313/v314 W6 increment 1 (reward programme identity + switchboard inversion) + v322 owner programme rulings + v325 business bio + v326/v326a points-gift lifecycle + v327 customer branch choice + v327 global customer QR (parallel-session v327 number collision) + v328 staff-choice manual confirm + v329 owner reschedule booking request + v330 pending slot block & confirmation template
  ...
  assert.equal(
    ...
    'nestly_v330_pending_slot_block_and_confirmation_template' // tail
  );
