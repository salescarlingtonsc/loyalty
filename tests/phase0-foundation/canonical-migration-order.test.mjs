...
test('checked-in canonical plan preserves 45 trusted catalog versions then 140 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 185-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 140);
  assert.equal(manifest.itemCount, 185);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 185);
