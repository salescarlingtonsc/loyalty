...
test('checked-in canonical plan preserves 45 trusted catalog versions then 81 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 126-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 81);
  assert.equal(manifest.itemCount, 126);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 126);
  assert.equal(manifest.items[44].version, '20260719190540');
  assert.equal(manifest.items[45].version, '20260721000001');
  assert.equal(manifest.items.at(-1).name, 'nestly_v86_enterprise_sme_crm');
