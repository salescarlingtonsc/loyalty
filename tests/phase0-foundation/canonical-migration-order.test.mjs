...
test('checked-in canonical plan preserves 45 trusted catalog versions then 225 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 270-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 225); // V263
  assert.equal(manifest.itemCount, 270); // V263
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 270); // V263
  ...
  assert.equal(
    ...
    'nestly_v267_salesforce_crm_scope'
  );
