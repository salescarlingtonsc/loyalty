...
test('materializer creates one byte-preserving 130-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 85);
  assert.equal(manifest.itemCount, 130);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 130);
  ...
  assert.equal(
    ...
    'nestly_v90_production_readiness'
  );
