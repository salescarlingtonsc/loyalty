...
test('checked-in canonical plan preserves 45 trusted catalog versions then 168 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 213-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 168);
  assert.equal(manifest.itemCount, 213);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 213);
  ...
  assert.equal(
    ...
    'nestly_v184_product_service_sectors'
  );
