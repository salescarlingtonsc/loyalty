...
test('checked-in canonical plan preserves 45 trusted catalog versions then 166 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 211-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 166);
  assert.equal(manifest.itemCount, 211);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 211);
  ...
  assert.equal(
    ...
    'nestly_v182_birthday_month_and_tier_schedule'
  );
