...
test('checked-in canonical plan preserves 45 trusted catalog versions then 209 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 254-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 209);
  assert.equal(manifest.itemCount, 254);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 254);
  ...
  assert.equal(
    ...
    'nestly_v242_customer_business_directory'
  );
