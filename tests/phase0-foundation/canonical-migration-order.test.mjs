...
test('materializer creates one byte-preserving 279-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 234); // V279
  assert.equal(manifest.itemCount, 279); // V279
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 279); // V279
  ...
  assert.equal(
    ...
    'nestly_v280_branch_billing_units_and_promotion_version' // V279
  );
