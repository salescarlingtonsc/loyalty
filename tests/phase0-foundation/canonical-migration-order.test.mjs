...
test('checked-in canonical plan preserves 45 trusted catalog versions then 117 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 162-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 117);
  assert.equal(manifest.itemCount, 162);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 162);
  ...
  assert.equal(
    ...
    'nestly_v122_owner_seven_workflows'
  );
