...
test('checked-in canonical plan preserves 45 trusted catalog versions then 197 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 242-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 197);
  assert.equal(manifest.itemCount, 242);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 242);
