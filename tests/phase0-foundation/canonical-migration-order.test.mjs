...
test('checked-in canonical plan preserves 45 trusted catalog versions then 192 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 237-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 192);
  assert.equal(manifest.itemCount, 237);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 237);
  ...
  assert.equal(
    ...
    'nestly_v213_sales_mix_audit_column'
  );
