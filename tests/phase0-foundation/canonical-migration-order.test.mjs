...
test('checked-in canonical plan preserves 45 trusted catalog versions then 183 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 228-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 183);
  assert.equal(manifest.itemCount, 228);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 228);
  ...
  assert.equal(
    ...
    'nestly_v204_bundle_priced_by_the_server'
  );
