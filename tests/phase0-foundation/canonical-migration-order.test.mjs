...
test('materializer creates one byte-preserving 283-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 239); // V284 customer-intelligence grants
  assert.equal(manifest.itemCount, 284); // V284 customer-intelligence grants
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 284); // V284 customer-intelligence grants
  ...
  assert.equal(
    ...
    'nestly_v284_customer_intelligence_v83' // V284
  );
