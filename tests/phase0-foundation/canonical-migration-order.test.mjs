...
test('checked-in canonical plan preserves 45 trusted catalog versions then 111 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 156-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 111);
  assert.equal(manifest.itemCount, 156);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 156);
  ...
  assert.equal(
    ...
    'nestly_v114_traceable_growth_costs'
  );
