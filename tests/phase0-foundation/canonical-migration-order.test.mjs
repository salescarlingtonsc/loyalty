...
test('materializer creates one byte-preserving 272-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 227); // V273
  assert.equal(manifest.itemCount, 272); // V273
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 272); // V273
  ...
  assert.equal(
    ...
    'nestly_v273_usage_null_when_never_set_up' // V273
  );
