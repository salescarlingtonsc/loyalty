...
test('materializer creates one byte-preserving 283-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 239); // V283 customer-claim grants
  assert.equal(manifest.itemCount, 284); // V283 customer-claim grants
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 284); // V283 customer-claim grants
  ...
  assert.equal(
    ...
    'nestly_v284_comms_foundation' // V284
  );
