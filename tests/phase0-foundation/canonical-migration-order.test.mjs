...
test('materializer creates one byte-preserving 297-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 252); // + V297-V299 prospecting CRM
  assert.equal(manifest.itemCount, 297); // + V297-V299 prospecting CRM
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 297); // + V297-V299 prospecting CRM
