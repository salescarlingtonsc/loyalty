...
test('checked-in canonical plan preserves 45 trusted catalog versions then 188 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 233-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 188);
  assert.equal(manifest.itemCount, 233);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 233);
  ...
  assert.equal(
    ...
    'nestly_v200a_subscription_revenue_schedule'
  );
