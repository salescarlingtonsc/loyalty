...
test('materializer creates one byte-preserving 286-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 242); // V283 customer-claim grants
  assert.equal(manifest.itemCount, 287); // V283 customer-claim grants
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 287); // V283 customer-claim grants
  ...
  assert.equal(
    ...
    'nestly_v289_reschedule_respects_business_setting' // V289
  );
