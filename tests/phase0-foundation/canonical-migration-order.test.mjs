...
test('materializer creates one byte-preserving 88-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 47);
  assert.equal(manifest.itemCount, 92);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 92);
  assert.equal(manifest.items[44].version, '20260719190540');
  assert.equal(manifest.items[45].version, '20260721000001');
  assert.equal(manifest.items.at(-1).name, 'frenly_v58_ps1c_checkout_kernel');
