...
test('checked-in canonical plan preserves 45 trusted catalog versions then 182 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 227-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 182);
  assert.equal(manifest.itemCount, 227);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 227);
  ...
  assert.equal(
    ...
    'nestly_v202b_branch_insert_only_via_paid_rpc'
  );
