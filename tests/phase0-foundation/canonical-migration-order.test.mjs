...
test('checked-in canonical plan preserves 45 trusted catalog versions then 145 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 190-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 145);
  assert.equal(manifest.itemCount, 190);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 190);
  ...
  assert.equal(
    ...
    'nestly_v164_account_signup_triage'
  );
