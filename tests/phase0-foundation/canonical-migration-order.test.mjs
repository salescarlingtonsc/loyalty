...
test('checked-in canonical plan preserves 45 trusted catalog versions then 88 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 133-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 88);
  assert.equal(manifest.itemCount, 133);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 133);
  ...
  assert.equal(
    ...
    'nestly_v93_branch_scoped_merchant_redemption'
  );
