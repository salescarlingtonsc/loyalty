...
test('checked-in canonical plan preserves 45 trusted catalog versions then 194 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 239-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 194);
  assert.equal(manifest.itemCount, 239);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 239);
  ...
  assert.equal(
    ...
    'nestly_v217_recent_window_and_staff_reference_code'
  );
