...
test('checked-in canonical plan preserves 45 trusted catalog versions then 98 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 143-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 98);
  assert.equal(manifest.itemCount, 143);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 143);
  ...
  assert.equal(
    ...
    'nestly_v105_admin_task_closure'
  );
