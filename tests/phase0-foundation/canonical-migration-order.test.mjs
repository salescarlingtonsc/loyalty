...
test('materializer creates one byte-preserving 301-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 256); // + v310 google content retention
  assert.equal(manifest.itemCount, 301); // + v310 google content retention
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 301); // + v310 google content retention
