...
test('checked-in canonical plan preserves 45 trusted catalog versions then 211 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 256-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 211);
  assert.equal(manifest.itemCount, 256);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 256);
  ...
  assert.equal(
    ...
    'nestly_v244_retention_audience_server_side'
  );
