...
test('materializer creates one byte-preserving 550-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(
    ...
    'nestly_v688_support_mark_read' // tail — v688 support inbox mark-read, with v687 staff self-profile before it, v686 hard-deleting a service is owner-only at the RPC (audit F068; renumbered from v682 after a slot collision), v683 referral new-customers-only, v684 delete retired offer, v685 Singapore day authority
    // tail was previously — nestly_v686_service_delete_owner_only_rpc — v686 hard-deleting a service is owner-only at the RPC (audit F068; renumbered from v682 after a slot collision) — with v683 referral new-customers-only, v684 delete retired offer, v685 Singapore day authority
    // tail was previously — nestly_v682_custom_perk_is_not_a_free_gift — v682 a written 'custom' perk is not a no-purchase gift (owner ruling)
    // tail was previously — v681 a scanned gift that needs no purchase is confirmed yes/no at the keypad
    // tail was previously — v680 a verified manual payment reopens the workspace (audit F129)
    // tail was previously — v673 staff_insert is owner-only (audit F132) — v673 staff_insert is owner-only (audit F132)
    // tail was previously — v672 the public support desk — v672 the public support desk (/support -> the super-admin queue)
    // tail was previously — v669 numeric honesty — v669 numeric honesty (Newcombe hybrid Wilson CI + honest null median)
