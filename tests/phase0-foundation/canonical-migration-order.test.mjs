...
test('checked-in canonical plan preserves 45 trusted catalog versions then 158 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 203-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 158);
  assert.equal(manifest.itemCount, 203);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 203);
  ...
  assert.equal(
    ...
    'nestly_v178_dispatch_secret_db_verify'
  );
