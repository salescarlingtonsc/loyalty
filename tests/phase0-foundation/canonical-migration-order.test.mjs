...
test('checked-in canonical plan preserves 45 trusted catalog versions then 200 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 245-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 200);
  assert.equal(manifest.itemCount, 245);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 245);
  ...
  assert.equal(
    ...
    'nestly_v230_customer_sees_the_chosen_points_mode'
  );
