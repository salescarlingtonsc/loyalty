...
test('checked-in canonical plan preserves 45 trusted catalog versions then 234 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 447-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(
    ...
    'nestly_v580_appointment_send_follows_the_appointment' // tail — a cancelled appointment sends nothing
  );
