...
test('checked-in canonical plan preserves 45 trusted catalog versions then 163 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 208-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 163);
  assert.equal(manifest.itemCount, 208);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 208);
  ...
  assert.equal(
    ...
    'nestly_v176b_reward_tier_gate_read_path'
  );
