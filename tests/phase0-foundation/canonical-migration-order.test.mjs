...
test('checked-in canonical plan preserves 45 trusted catalog versions then 472 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 539-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(
    ...
    'nestly_v744_scanner_blind_spots' // tail — v744 scanner blind spots, after v743 estate-wide synthetic scanner
