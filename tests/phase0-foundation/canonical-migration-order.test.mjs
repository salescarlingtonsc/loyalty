...
test('materializer creates one byte-preserving 278-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 233); // V279
  assert.equal(manifest.itemCount, 278); // V279
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 278); // V279
