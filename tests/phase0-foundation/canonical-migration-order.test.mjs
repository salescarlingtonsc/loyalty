...
test('materializer creates one byte-preserving 289-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 244); // +v288 +v289
  assert.equal(manifest.itemCount, 289); // +v288 +v289
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 289); // +v288 +v289
  ...
  assert.equal(
    ...
    'nestly_v290_customer_withdraw_booking_request' // V290
  );
