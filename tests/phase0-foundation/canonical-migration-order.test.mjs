...
test('checked-in canonical plan preserves 45 trusted catalog versions then 460 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 505-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(
    ...
    'nestly_v634_reconcile_failure_visibility' // tail — reconcile-failure visibility (P0 hardening) // tail — the two automation control-plane writers
