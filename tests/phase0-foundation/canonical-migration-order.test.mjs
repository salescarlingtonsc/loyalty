...
test('materializer creates one byte-preserving 285-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 241); // V283 customer-claim grants
  assert.equal(manifest.itemCount, 286); // V283 customer-claim grants
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 286); // V283 customer-claim grants
  ...
  assert.equal(
    ...
    'nestly_v285_a4_gap_closure' // V285
  );
