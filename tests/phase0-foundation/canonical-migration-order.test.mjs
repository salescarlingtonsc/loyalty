...
test('materializer creates one byte-preserving 268-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 223); // V263
  assert.equal(manifest.itemCount, 268); // V263
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 268); // V263
  ...
  assert.equal(
    ...
    'nestly_v264_promotion_share_event' // V263
  );
