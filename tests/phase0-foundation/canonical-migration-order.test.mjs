...
test('checked-in canonical plan preserves 45 trusted catalog versions then 231 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 440-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(
    ...
    'nestly_v573_module_off_reaches_the_rpcs' // tail — retention lane inert repair
  );
