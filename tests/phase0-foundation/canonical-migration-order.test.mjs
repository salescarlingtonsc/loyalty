...
test('materializer creates one byte-preserving 129-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 84);
  assert.equal(manifest.itemCount, 129);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 129);
  ...
  assert.equal(
    ...
    'nestly_v89_customer_qr_redemption_platform_access'
  );
