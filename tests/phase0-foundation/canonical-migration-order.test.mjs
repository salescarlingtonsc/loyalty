...
test('materializer creates one byte-preserving 304-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 259); // + v311 money kernel + v312 pot migration
  assert.equal(manifest.itemCount, 304); // + v311 money kernel + v312 pot migration
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 304); // + v311 money kernel + v312 pot migration
  ...
  assert.equal(
    ...
    'nestly_v312_programme_pot_migration' // tail
  );
