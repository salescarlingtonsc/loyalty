...
test('checked-in canonical plan preserves 45 trusted catalog versions then 118 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 163-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 118);
  assert.equal(manifest.itemCount, 163);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 163);
  ...
  assert.equal(
    ...
    'nestly_v123_module_button_readiness'
  );
