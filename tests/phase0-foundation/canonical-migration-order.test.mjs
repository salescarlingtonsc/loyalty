...
test('checked-in canonical plan preserves 45 trusted catalog versions then 206 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 251-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 206);
  assert.equal(manifest.itemCount, 251);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 251);
  ...
  assert.equal(
    ...
    'nestly_v226_crm_consultant_scope'
  );
