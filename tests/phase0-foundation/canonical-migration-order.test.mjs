...
test('checked-in canonical plan preserves 45 trusted catalog versions then 160 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 205-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 160);
  assert.equal(manifest.itemCount, 205);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 205);
  ...
  assert.equal(
    ...
    'nestly_v179_report_insight_evidence'
  );
