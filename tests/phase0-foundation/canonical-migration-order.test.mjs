...
test('materializer creates one byte-preserving 294-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 249); // V306 both-mode tiers capability
  assert.equal(manifest.itemCount, 294); // V306 both-mode tiers capability
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 294); // V306 both-mode tiers capability
  ...
  assert.equal(
    ...
    'nestly_v306_both_mode_shows_tiers' // tail
  );
