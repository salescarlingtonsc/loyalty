...
test('checked-in canonical plan preserves 45 trusted catalog versions then 196 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 241-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 196);
  assert.equal(manifest.itemCount, 241);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 241);
  ...
  assert.equal(
    ...
    'nestly_v220_next_best_times_for_selected_staff'
  );
