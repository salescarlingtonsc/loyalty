...
test('checked-in canonical plan preserves 45 trusted catalog versions then 130 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 175-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 130);
  assert.equal(manifest.itemCount, 175);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 175);
  ...
  assert.equal(
    ...
    'nestly_v143_effective_reward_tier_boundaries'
  );
