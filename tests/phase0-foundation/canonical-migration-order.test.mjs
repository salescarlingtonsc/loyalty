...
test('checked-in canonical plan preserves 45 trusted catalog versions then 124 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 169-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 124);
  assert.equal(manifest.itemCount, 169);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 169);
  ...
  assert.equal(
    ...
    'nestly_v130_self_serve_business_onboarding'
  );
