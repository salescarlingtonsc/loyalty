...
test('materializer creates one byte-preserving 448451 and deterministic manifests', async (t) => {
  ...
  assert.equal(
    ...
    'nestly_v599_browser_write_boundaries' // tail — the browser roles lose the writes nothing in the product uses
    // previously: nestly_v598_shop_hours_are_the_default // the shop's opening hours are every teammate's default
  );
