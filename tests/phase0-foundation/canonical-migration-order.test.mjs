...
test('checked-in canonical plan preserves 45 trusted catalog versions then 189 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 234-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 189);
  assert.equal(manifest.itemCount, 234);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 234);
  ...
  assert.equal(
    ...
    'nestly_v201_receipt_storage_policies'
  );
