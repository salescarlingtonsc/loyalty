...
test('checked-in canonical plan preserves 45 trusted catalog versions then 193 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 238-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 193);
  assert.equal(manifest.itemCount, 238);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 238);
  ...
  assert.equal(
    ...
    'nestly_v215_signup_welcome_offer'
  );
