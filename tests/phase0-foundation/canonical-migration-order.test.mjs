...
test('checked-in canonical plan preserves 45 trusted catalog versions then 94 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 139-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 94);
  assert.equal(manifest.itemCount, 139);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 139);
  ...
  assert.equal(
    ...
    'nestly_v100_product_adoption_events'
  );
