...
test('checked-in canonical plan preserves 45 trusted catalog versions then 471 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 529-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(
    ...
    'nestly_v666_add_branch_tier_capacity' // tail — v666 adding a branch prices from the tier ladder
