...
test('materializer creates one byte-preserving 282-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 238); // V283 customer-claim grants
  assert.equal(manifest.itemCount, 283); // V283 customer-claim grants
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 283); // V283 customer-claim grants
  ...
  assert.equal(
    ...
    'nestly_v283_customer_claim_execute_grants' // V283
  );
