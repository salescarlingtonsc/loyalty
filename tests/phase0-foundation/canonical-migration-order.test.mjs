...
test('checked-in canonical plan preserves 45 trusted catalog versions then 230 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 437-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(
    ...
    'nestly_v570_dashboard_off_means_off' // tail — a module set Off is off at the reader
  );
