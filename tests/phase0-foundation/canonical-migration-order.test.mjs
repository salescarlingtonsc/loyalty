...
test('checked-in canonical plan preserves 45 trusted catalog versions then 198 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 243-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 198);
  assert.equal(manifest.itemCount, 243);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 243);
  ...
  assert.equal(
    ...
    'nestly_v223_table_reservations_opt_in'
  );
