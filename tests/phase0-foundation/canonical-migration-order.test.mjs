...
test('materializer creates one byte-preserving 270-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 225); // V267
  assert.equal(manifest.itemCount, 270); // V267
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 270); // V267
  ...
  assert.equal(
    ...
    'nestly_v267_summary_business_logo' // V267
  );
