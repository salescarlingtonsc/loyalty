...
test('checked-in canonical plan preserves 45 trusted catalog versions then 89 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 134-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 89);
  assert.equal(manifest.itemCount, 134);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 134);
  ...
  assert.equal(
    ...
    'nestly_v94_platform_control_intelligence'
  );
