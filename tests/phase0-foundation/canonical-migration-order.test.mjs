...
test('checked-in canonical plan preserves 45 trusted catalog versions then 153 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 198-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 153);
  assert.equal(manifest.itemCount, 198);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 198);
