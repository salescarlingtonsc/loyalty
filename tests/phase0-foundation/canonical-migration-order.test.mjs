...
test('checked-in canonical plan preserves 45 trusted catalog versions then 91 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 136-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 91);
  assert.equal(manifest.itemCount, 136);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 136);
  ...
  assert.equal(
    ...
    'nestly_v96_customer_programme_selector_media'
  );
