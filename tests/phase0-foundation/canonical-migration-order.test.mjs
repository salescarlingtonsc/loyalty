...
test('checked-in canonical plan preserves 45 trusted catalog versions then 150 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 195-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 150);
  assert.equal(manifest.itemCount, 195);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 195);
  ...
  assert.equal(
    ...
    'nestly_v170_staff_update_client'
  );
