...
test('checked-in canonical plan preserves 45 trusted catalog versions then 202 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 247-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 202);
  assert.equal(manifest.itemCount, 247);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 247);
  ...
  assert.equal(
    ...
    'nestly_v232_promotion_lock_cannot_take_down_the_api'
  );
