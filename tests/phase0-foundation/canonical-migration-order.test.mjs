...
test('checked-in canonical plan preserves 45 trusted catalog versions then 191 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 236-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 191);
  assert.equal(manifest.itemCount, 236);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 236);
  ...
  assert.equal(
    ...
    'nestly_v209_static_single_join_qr'
  );
