...
test('checked-in canonical plan preserves 45 trusted catalog versions then 212 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 257-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 212);
  assert.equal(manifest.itemCount, 257);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 257);
  ...
  assert.equal(
    ...
    'nestly_v245_customer_explore_search'
  );
