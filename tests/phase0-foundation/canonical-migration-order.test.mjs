...
test('checked-in canonical plan preserves 45 trusted catalog versions then 151 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 196-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 151);
  assert.equal(manifest.itemCount, 196);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 196);
  ...
  assert.equal(
    ...
    'nestly_v171_customerintel_entitlement'
  );
