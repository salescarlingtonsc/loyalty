...
test('materializer creates one byte-preserving 284-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 240); // V283 customer-claim grants
  assert.equal(manifest.itemCount, 285); // V283 customer-claim grants
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 285); // V283 customer-claim grants
  ...
  assert.equal(
    ...
    'nestly_v285_offer_share_imageless_parity' // V285
  );
