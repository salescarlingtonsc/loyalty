...
test('checked-in canonical plan preserves 45 trusted catalog versions then 121 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 166-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 121);
  assert.equal(manifest.itemCount, 166);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 166);
  ...
  assert.equal(
    ...
    'nestly_v127_rewards_overview_birthday_reader'
  );
