...
test('checked-in canonical plan preserves 45 trusted catalog versions then 155 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 200-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 155);
  assert.equal(manifest.itemCount, 200);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 200);
