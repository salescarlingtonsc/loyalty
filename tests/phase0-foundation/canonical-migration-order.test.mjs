...
test('checked-in canonical plan preserves 45 trusted catalog versions then 90 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 135-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 90);
  assert.equal(manifest.itemCount, 135);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 135);
  ...
  assert.equal(
    ...
    'nestly_v95_bilingual_programmes'
  );
