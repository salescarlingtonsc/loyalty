...
test('checked-in canonical plan preserves 45 trusted catalog versions then 131 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 176-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 131);
  assert.equal(manifest.itemCount, 176);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 176);
  ...
  assert.equal(
    ...
    'nestly_v145_launch_freeze_metrics'
  );
