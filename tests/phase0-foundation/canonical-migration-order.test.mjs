...
test('materializer creates one byte-preserving 290-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 245); // +v288 +v289
  assert.equal(manifest.itemCount, 290); // +v288 +v289
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 290); // +v288 +v289
  ...
  assert.equal(
    ...
    'nestly_v290_server_debt_closure' // tail
  );
