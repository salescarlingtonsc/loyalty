...
test('checked-in canonical plan preserves 45 trusted catalog versions then 199 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 244-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 199);
  assert.equal(manifest.itemCount, 244);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 244);
  ...
  assert.equal(
    ...
    'nestly_v229_points_mode_choice'
  );
