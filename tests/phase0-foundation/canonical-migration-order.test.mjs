...
test('checked-in canonical plan preserves 45 trusted catalog versions then 147 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 192-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 147);
  assert.equal(manifest.itemCount, 192);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 192);
  ...
  assert.equal(
    ...
    'nestly_v168_approved_account_signup_admission'
  );
