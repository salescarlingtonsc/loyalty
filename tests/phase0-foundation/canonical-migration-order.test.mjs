...
test('checked-in canonical plan preserves 45 trusted catalog versions then 119 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 164-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 119);
  assert.equal(manifest.itemCount, 164);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 164);
  ...
  assert.equal(
    ...
    'nestly_v124_stripe_launch_pricing'
  );
