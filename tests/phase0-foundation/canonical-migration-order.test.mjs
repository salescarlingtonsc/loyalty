...
test('checked-in canonical plan preserves 45 trusted catalog versions then 177 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 222-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(manifest.pendingCount, 177);
  assert.equal(manifest.itemCount, 222);
  assert.equal(new Set(manifest.items.map(({ version }) => version)).size, 222);
  ...
  assert.equal(
    ...
    'nestly_v197_recommendation_duplicate_root_cause'
  );
