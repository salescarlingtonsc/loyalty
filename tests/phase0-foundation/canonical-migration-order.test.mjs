...
test('checked-in canonical plan preserves 45 trusted catalog versions then 459 unique pending versions', () => {
  const result = run(repoRoot, '--check-plan');
  assert.equal(result.status, 0, result.stderr);
});

test('materializer creates one byte-preserving 504-file chain and deterministic manifests', async (t) => {
  ...
  assert.equal(
    ...
    'nestly_v632_inbox_mark_all_read' // tail — packages and products can be limited to branches, and a pending request can be amended // tail — the two automation control-plane writers
