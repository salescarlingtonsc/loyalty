...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  assert.equal(manifest.itemCount, 303); // + v311 money kernel + v312 pot migration
  assert.equal(manifest.executableCount, 289); // + v311 money kernel + v312 pot migration
  assert.equal(manifest.reservationCount, 14);
  assert.equal(sqlItems.length, 289); // + v311 money kernel + v312 pot migration
