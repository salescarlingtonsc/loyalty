...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  assert.equal(manifest.itemCount, 289); // V283 customer-claim grants
  assert.equal(manifest.executableCount, 275); // V283 customer-claim grants
  assert.equal(manifest.reservationCount, 14);
  assert.equal(sqlItems.length, 275); // V283 customer-claim grants
