...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  assert.equal(manifest.itemCount, 293); // V306 both-mode tiers capability
  assert.equal(manifest.executableCount, 279); // V306 both-mode tiers capability
  assert.equal(manifest.reservationCount, 14);
  assert.equal(sqlItems.length, 279); // V306 both-mode tiers capability
  ...
  const expectedCollisionCounts = new Map([
    ...
    ['20260811', 6], // V279
    ['20260812', 12], // +v293 customer-intelligence grants
