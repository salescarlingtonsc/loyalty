...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  const expectedCollisionCounts = new Map([
    ...
    ['20260822', 14], // v421 two-sided referral + v422 customer reward history + the 2026-08-22 rewards go-live wave (v423 reward edit reaches customers, v424 birthday window honoured, v425 referral explicit reward type, v426 canonical tier resolver, v427 entitlements reach customers, v432 staff redeem-now matches the customer) share a file-name day
  ]);
  assert.deepEqual(
