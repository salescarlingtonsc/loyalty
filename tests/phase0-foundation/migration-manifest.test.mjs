...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  assert.equal(manifest.itemCount, 308); // + v311 money kernel + v312 pot migration + v315 lead score repair + v316 taxonomy/match queue repair
  assert.equal(manifest.executableCount, 294); // + v311 money kernel + v312 pot migration + v315 lead score repair + v316 taxonomy/match queue repair
  assert.equal(manifest.reservationCount, 14);
  assert.equal(sqlItems.length, 294); // + v311 money kernel + v312 pot migration + v315 lead score repair + v316 taxonomy/match queue repair
  ...
  const expectedCollisionCounts = new Map([
    ...
    ['20260814', 5] // V313 conversion-first prospecting + its recovery snapshot + V314 business explorer + V315 lead score repair + V316 taxonomy/match queue repair
  ]);
  assert.deepEqual(
