...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  const expectedCollisionCounts = new Map([
    ...
    ['20260821', 7] // v403 ledger token + v404 manual redemption + v409 canonical balance + v410 + v412 + v414 stamp card length share a file-name day
  ]);
  assert.deepEqual(
