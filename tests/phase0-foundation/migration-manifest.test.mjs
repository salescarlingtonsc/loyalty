...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  assert.equal(manifest.itemCount, 290); // V283 customer-claim grants
  assert.equal(manifest.executableCount, 276); // V283 customer-claim grants
  assert.equal(manifest.reservationCount, 14);
  assert.equal(sqlItems.length, 276); // V283 customer-claim grants
