...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  const expectedCollisionCounts = new Map([
    ...
    ['20260920', 60],
    // v754 (gift-expiry-is-a-redeem-by-date) is dated 20260923, the same source-file day as v748
    // (history-states-points-expiry) — two files, not a collision (different filenames).
    ['20260923', 2],
  ]);
  assert.deepEqual(
