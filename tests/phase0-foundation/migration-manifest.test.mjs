...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  assert.equal(manifest.itemCount, 310); // + v311 money kernel + v312 pot migration + v315 lead score repair + v316 taxonomy/match queue repair + v317 restored dependencies + v318 system-managed flag alignment
  assert.equal(manifest.executableCount, 296); // + v311 money kernel + v312 pot migration + v315 lead score repair + v316 taxonomy/match queue repair + v317 restored dependencies + v318 system-managed flag alignment
  assert.equal(manifest.reservationCount, 14);
  assert.equal(sqlItems.length, 296); // + v311 money kernel + v312 pot migration + v315 lead score repair + v316 taxonomy/match queue repair + v317 restored dependencies + v318 system-managed flag alignment
  ...
  const expectedCollisionCounts = new Map([
    ...
    ['20260814', 7] // V313 conversion-first prospecting + its recovery snapshot + V314 business explorer + V315 lead score repair + V316 taxonomy/match queue repair + V317 restored dependencies + V318 system-managed flag alignment
  ]);
  assert.deepEqual(
