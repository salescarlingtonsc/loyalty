...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  assert.equal(manifest.itemCount, 283); // V284 customer-intelligence grants
  assert.equal(manifest.executableCount, 269); // V284 customer-intelligence grants
  assert.equal(manifest.reservationCount, 14);
  assert.equal(sqlItems.length, 269); // V284 customer-intelligence grants
  ...
  const expectedCollisionCounts = new Map([
    ...
    ['20260811', 6], // V279
    ['20260812', 4] // V281 stripe + V282 fast-fail + V283 claim grants + V284 intelligence grants
  ]);
  assert.deepEqual(
