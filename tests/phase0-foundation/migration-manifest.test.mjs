...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  assert.equal(manifest.executableCount, 303); // + v311 money kernel + v312 pot migration + v315 lead score repair + v316 taxonomy/match queue repair + v317 restored dependencies + v318 system-managed flag alignment + v313/v314 W6 increment 1 (reward programme identity + switchboard inversion) + v322 owner programme rulings + v326/v326a points-gift lifecycle
  assert.equal(manifest.reservationCount, 14);
  assert.equal(sqlItems.length, 303); // + v311 money kernel + v312 pot migration + v315 lead score repair + v316 taxonomy/match queue repair + v317 restored dependencies + v318 system-managed flag alignment + v313/v314 W6 increment 1 (reward programme identity + switchboard inversion) + v322 owner programme rulings + v326/v326a points-gift lifecycle
  ...
  const expectedCollisionCounts = new Map([
    ...
    ['20260815', 3] // v326 points-gift lifecycle + v326a anon-execute revoke + v327 customer branch choice
  ]);
  assert.deepEqual(
