...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  const expectedCollisionCounts = new Map([
    ...
    ['20260829', 10],  // v602 same-business composite FKs + v601 pause the nine featureless cron jobs + v600 register v361 bring-back cron + v599 browser write boundaries + v586 tier basis + v587 join QR + v588 staff signup + v589 referral one-switch + v593 package expiry + v598 shop hours are the default
  ]);
  assert.deepEqual(
