...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  const expectedCollisionCounts = new Map([
    ...
    ['20260823', 4], // v463 stamp card max (R3a) + v465 Home ready count (R1) + v464 earned reward expiry (R3e) + v462 featured offer & live cap (R2) share a file-name day
  ]);
  assert.deepEqual(
