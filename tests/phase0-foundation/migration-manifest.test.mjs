...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  const expectedCollisionCounts = new Map([
    ...
    ['20260920', 60],
    // v750 (a kept bottle no longer blocks account deletion) joins v749 (customer self-service
    // account deletion) on 20260924, so that day becomes a collision for the first time; the
    // deploy versions (…000000 and …000001) keep them strictly ordered.
    ['20260924', 5], // + v751 capped-discount wording, v752 birthday gift is a real benefit, v754 gift expiry is a redeem-by date (owner batch 2) — deploy slots 000000, 000001, 010000, 020000, 040000 keep them strictly ordered
  ]);
  assert.deepEqual(
