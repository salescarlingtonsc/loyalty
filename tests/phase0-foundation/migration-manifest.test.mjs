...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  const expectedCollisionCounts = new Map([
    ...
    ['20260831', 19], ['20260901', 6], ['20260902', 17], // (v801-v808 audit wave 4 files carry their 20261006 deploy day, not this one) (appointment change integrity, stamp-gift reversal + pin, membership pause + replay markers, pot-scope + unpaged totals, stamp-gift delete version-forward, guest booking auto-approve, reschedule un-assign staff) + v674-v680 audit P1 wave (six) + v672 public support desk + v673 staff_insert owner-only (F132) // + v665 branch subscription choice + v666 branch add prices from the tier ladder // + v664 capacity tiers charged per branch, manual-payment billing dates // + v659 edit means edit // + v656 tier discount scope + v655 cancel and package edit // v654 receipt identity grant & tier-perk last_used_at + v651 canonical cadence + v652 evidence contract (Phase D) + v644 can-contact authority + v645 messaging retention + v646 engagement rollups + v647 canonical taxonomy + v648 service canonical map + v649 category snapshot at sale + v650 CI read layer (Phase B/C)
    // The CI-100 proof wave (v672-v744, sixty files) is dated 20260920 so the local harness applies it
    // after main's v672-v688 audit waves, matching its deploy slots 20260920000000-20260922110000
    // (merged 2026-09-03; the semantic numbers are twins of main's by filename only).
    ['20260920', 60],
    // v751 (deleted-phone marks block welcome-offer/referral rejoin) joins v750 (a kept bottle
    // no longer blocks account deletion) and v749 (customer self-service account deletion) on
    // 20260924, raising the collision from 2 to 3; the deploy versions (…000000, …000001,
    // …000002) keep them strictly ordered.
    ['20260924', 7] /* + batch 2: v751 wording, v752 birthday benefit, v754 redeem-by date at slots 010000/020000/040000 */,
    ['20260929', 2] /* v759 weekly breaks + v760 a Razorpay customer may span several businesses */,
    ['20260930', 2] /* v763 self-serve activation by tier (000000) + v764 birthday rejoin guard (010000) — parallel sessions on the same day */,
    ['20261002', 2] /* v766 self-serve activation tier price (000000) + v767 referral link joins (010000) */,
    ['20261003', 3] /* v768 Keep on a paid stopping branch (000000) + v772 owner-brief readers (010000) + v775 visit-rhythm totals (020000) — three sessions on one day, twin-named v772/v775 with the branch-copy and capacity-increase migrations, not a real conflict */,
    ['20261006', 25] /* + v798 (231000) */ /* + v801-v808 audit wave 4 (160000-230000) */ /* + v797 (150000) */ /* + v794 (140000) */ /* + v793 (130000) */ /* + v792 (120000) */ /* + v791 (110000) */ /* + v790 (100000) */ /* + v788 branch receipt identity + GST (090000) */ /* + v787 live firms on the pipeline (080000) */ /* + v784 (060000) + v786 (070000) */ /* + v785 pipeline schedule (050000) */ /* + v779 platform business payments (040000) */ /* v775 capacity increase invoice reason (000000) + v777 referral reward visible (010000) + v777 branch code and comparison (020000) + v779 no weekday without visits (030000) — three parallel sessions on one day, two of them twin-named v777 */,
    ['20261007', 19]  /* + v897 a bar is an F&B business (20261010210000) */  /* + v895 services map themselves on creation (file dated 20261007, deploy slot 20261010200000) */ /* + v825 the security hygiene sweep (audit P2, 20261007170000) */ /* + v826 the owner brief: nightly snapshot + get_owner_brief_v1 (160000) */ /* + v824 API WhatsApp sending off platform-wide, manual WhatsApp stays (150000) */ /* + v823 the nightly reconciliation call waits 60 s for a cold reconciler, not pg_net's 5 s default (140000) */ /* + v822 the Daily report counts one points pot (130000) */ /* + v821 the Stripe billing reconciler boots again, and a reconciler that fails or falls silent raises a dated alert */ /* + v820 the job decides who a customer can book, the owner still decides the person (110000) */ /* + v818 line commission, branch availability, booking sync (090000) + v819 merchant-settled reversal (100000) */ /* + v809 till card standing + expense note (000000) + v810 ACL + credit-note history (010000) + v811 self-serve activation always births the loyalty row (020000) + v813 wallet readers stay in their own pot scope (040000) + v814 stamp-card gift pause goes version-forward (050000) + v815 redeem across pots in business_pot scope (060000) + v816 WhatsApp sent_unconfirmed (070000) + v817 one CI gate completion (080000) */,
    ['20261008', 2] /* + v828 the owner brief answers the audit's open questions: nineteen more facts (20261008020000) */,
    ['20261009', 10]  /* + v879 paid means the contract was paid (20261010150000) *//* + v851 one overload per reward RPC, no anon execute, post-v744 reports exclude synthetic customers (20261009190000) */ /* + v832 the welcome-offer save respects the consumption mark (20261009020000) */ /* v831 a joining benefit is blocked by use, not by deletion (010000) joined by the audit P2 five: v845 stamp card reader truth (120000), v846 redemption intent agrees with the engine (130000), v847 redeem engine reversals expiry and spine (140000), v848 reversal returns the voucher and the books (150000), v849 catalogue truth and birthday warning (160000) — first collision on this source date, raising the count from 1 to 6 */,
    ['20261010', 18] /* + v884 a bundle is a service everywhere (20261010190000) */ /* + v883 firm billing schedule (20261010180000) */ /* + v882 booking past-start outcome, preferred_at expiry, bookable bundles (20261010170000) */ /* nestly_v865-v878 number-accuracy wave, slots 20261010010000-20261010140000 */,
    /* nestly_v894 the WhatsApp sign-up OTP (20261011000000) and nestly_v898 the template registry
       matching Meta (20261011010000) were written on the same day, so 20261011 is the first date
       prefix this repo has carried twice since the v895/v897 pair. */
    ['20261011', 3],
    /* nestly_v922 (the cadence switch, 20261012000000) and nestly_v928 (a live client's advisor,
       20261012010000) were written the same day, the second because driving the first on the
       deployed console found the v79 guard it had to go through. */
    ['20261012', 2],
    /* nestly_v951 (bounded read limits, 20261015000000) and nestly_v961 (promo codes,
       20261015010000) were written the same day by parallel sessions. */
    ['20261015', 2],
    /* nestly_v974 (the hourly template reconcile, 20261016010000) joins v962's stripe promo
       coupons on 20261016, making it the second date prefix this repo carries twice. */
    ['20261016', 8],
    /* nestly_v986 (20261019000000) and nestly_v987 (20261019010000) share the 20261019 source
       prefix on purpose: v987 exists only because v986's own acceptance suite found it -- the first
       honestly-recorded discounted invoice would not post to the ledger -- so they were written
       within minutes of each other and belong to one piece of work. */
    ['20261019', 4],
    /* nestly_v991 (no hours does not mean closed, 20261020000000) and nestly_v992 (approval is
       tested by every guard, 20261020010000) were written the same day by parallel sessions. */
    ['20261020', 2],
  ]);
  assert.deepEqual(
