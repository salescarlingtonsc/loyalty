...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  const expectedCollisionCounts = new Map([
    ...
    ['20260823', 3], // v463 stamp card max length (R3a) + v465 Home ready-reward count (R1) + v464 earned reward expiry (R3e) share a file-name day
  ]);
  assert.deepEqual(
