...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  const expectedCollisionCounts = new Map([
    ...
    ['20261006', 5] /* + v779 platform business payments (040000) */ /* v775 capacity increase invoice reason (000000) + v777 referral reward visible (010000) + v777 branch code and comparison (020000) + v779 no weekday without visits (030000) — three parallel sessions on one day, two of them twin-named v777 */,
  ]);
  assert.deepEqual(
