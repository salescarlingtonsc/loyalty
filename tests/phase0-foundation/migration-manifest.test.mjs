...
test('manifest covers every executable SQL file with raw-byte SHA-256 and a companion digest', async (t) => {
  ...
  assert.equal(manifest.executableCount, 269); // V283 customer-claim grants
  assert.equal(manifest.reservationCount, 14);
  assert.equal(sqlItems.length, 269); // V283 customer-claim grants
  ...
  const expectedCollisionCounts = new Map([
    ...
    ['20260811', 6], // V279
    ['20260812', 4] // V281 stripe + V282 promotion-finalize fast-fail + V283 customer-claim grants
  ]);
  assert.deepEqual(
