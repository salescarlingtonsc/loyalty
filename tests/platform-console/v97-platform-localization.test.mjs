...
test('runtime state, validation and announcement inventory cannot bypass localization',async()=>{
  ...
  assert.equal(explicit.length,663,'update the audited explicit-copy inventory when adding runtime UI');
  assert.equal(metadata.length,553,'update the audited CUI metadata inventory when adding UI metadata');
  assert.equal(announcements.length,32,'update the audited static announcement inventory when adding announcements');
...
test('mixed dynamic template inventory is explicitly classified',async()=>{
  ...
  assert.equal(classified.length,40,'review every interpolated template segment when the inventory changes');
