...
test('runtime state, validation and announcement inventory cannot bypass localization',async()=>{
  ...
  assert.equal(explicit.length,719,'update the audited explicit-copy inventory when adding runtime UI');
  assert.equal(metadata.length,610,'update the audited CUI metadata inventory when adding UI metadata');
  assert.equal(announcements.length,39,'update the audited static announcement inventory when adding announcements');
