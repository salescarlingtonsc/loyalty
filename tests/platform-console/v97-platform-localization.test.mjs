...
test('runtime state, validation and announcement inventory cannot bypass localization',async()=>{
  ...
  assert.equal(explicit.length,876,'update the audited explicit-copy inventory when adding runtime UI'); // V256
...
test('mixed dynamic template inventory is explicitly classified',async()=>{
  ...
  assert.equal(classified.length,45,'review every interpolated template segment when the inventory changes');
  ...
  assert.equal(classified.filter(entry=>entry.kind==='technical').length,36);
});
