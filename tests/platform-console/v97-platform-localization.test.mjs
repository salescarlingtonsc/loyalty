...
test('runtime state, validation and announcement inventory cannot bypass localization',async()=>{
  ...
  assert.equal(explicit.length,612,'update the audited explicit-copy inventory when adding runtime UI');
  assert.equal(metadata.length,477,'update the audited CUI metadata inventory when adding UI metadata');
  assert.equal(announcements.length,30,'update the audited static announcement inventory when adding announcements');
...
test('mixed dynamic template inventory is explicitly classified',async()=>{
  ...
  const classifications=[
    {kind:'localized-ui',pattern:/^disabled title="§"$/},
    {kind:'data-only',pattern:/^(?:§ (?:KB|MB)|Platform import review: §|NPU: §)$/},
    {kind:'technical',pattern:/^(?:nestly:v133:privacy:§:§|platform-§-§|platformNavGroup-§|cursor:§|#\/platform\/(?:firms|reports|onboarding)§|#\/platform\/(?:billing\?business|commissions\?accrual|automation\?incident)=§|#\/platform\/firms\?firm=§|module_mode_§|\[data-module-row="§"\] select|data-platform-scope="§"|\[data-(?:prospect|business-application|accrual-id|incident-id)="§"\]|§-enterprise-(?:report|cross-domain)-§-§\.csv|custom__§|§-import-§-errors\.csv|name="§"§+|§_cents|(?:business|prospect):§|permission_§)$/}
  ];
  const classified=inventory.map(entry=>({
    ...entry,kind:classifications.find(rule=>rule.pattern.test(entry.text))?.kind||'unclassified'
  }));
  assert.equal(classified.length,36,'review every interpolated template segment when the inventory changes');
  assert.deepEqual(classified.filter(entry=>entry.kind==='unclassified'),[]);
  assert.equal(classified.filter(entry=>entry.kind==='localized-ui').length,4);
  assert.equal(classified.filter(entry=>entry.kind==='data-only').length,4);
  assert.equal(classified.filter(entry=>entry.kind==='technical').length,28);
});
