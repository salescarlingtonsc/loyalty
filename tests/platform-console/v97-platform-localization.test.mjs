...
test('all static platform interface metadata has real locale copy and no code-like translation',async()=>{
  ...
  /* 'Import' joins these: it is the standard Malay word for import, exactly as 'CRM' is
     the standard term — an identity mapping is the CORRECT copy, not a missing one. */
  const invariants=new Set(['WhatsApp','Platform','UEN','CRM','Status','Import']);
...
test('runtime state, validation and announcement inventory cannot bypass localization',async()=>{
  ...
  assert.equal(metadata.length,903,'update the audited CUI metadata inventory when adding UI metadata'); // + v961 promo codes (+15: the page header and card titles, the create-code modal's field labels, and the error state) // + v922 planned cycle switch (+1: the 'New frequency' field label; its 'From' partner already existed as the Firms date-range label) // + v883 firm controls (+6: card titles, error/loading states) // v785 Pipeline (+14) + v779 payments by branch (+4) + F130 (+1) + v727 (+2), merged 2026-09-03
