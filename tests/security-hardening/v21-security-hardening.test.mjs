...
test('service-only allowlist plus v89/v95 replacements exactly cover the Edge Function call graph', async () => {
  const [migration, v89, v95, v183, v640, ...edgeSources] = await Promise.all([
    read(migrationPath),
    ...
    // v640 added the identity-free funnel-hit resolvers (Phase A / D2) the gateway fires
    // after successful join/booking page, availability and submit responses.
    read('db/migrations/20260830_nestly_v640_funnel_hit_resolvers.sql'),
  ...
  for (const name of serviceGrantNames(v640)) allowlist.add(name);
